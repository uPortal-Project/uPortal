...
public class DefinitionService extends AbstractJwtService {
    ...
    public Definition createDefinition(String title, String fname, String description,
                                       List<String> categories, Map<String,List<String>> parameters, String username, Date expires) {
        ...
        return new Definition(generateEncryptedToken(claims), title, fname, description, categories, parameters);
    ...
    public Definition parseDefinition(String definitionToken) {
        ...
        final List<String> categories = (List<String>) claims.getBody().get(JwtClaims.CATEGORIES.getName());
