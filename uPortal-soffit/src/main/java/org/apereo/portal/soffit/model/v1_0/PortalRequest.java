...
public class PortalRequest extends AbstractTokenizable {
  ...
  /**
   * Additional information that the portal (via the Soffit Connector) shares with the remote Soffit
   * will be set as attributes on the {@link PortalRequest}.
   *
   * @author drewwills
   */
  public enum Attributes {
    ...
    /**
     * A soffit may rely on this String to construct element IDs and selectors that are unique on
     * the page. The portal is responsible for issuing a namespace that does not conflict with any
     * other content on the same user's layout.
     *
     * <p><strong>WARNING:</strong> Soffits that use the GLOBAL cache option (cache across users)
     * may not rely on this strategy. There is no strategy that will work in such circumstances
     * short of generating a namespace in Javascript, since the same HTML output can be placed into
     * the DOM in multiple places.
     *
     * @since 5.0
     */
    NAMESPACE("namespace"),
    ...
    /**
     * Provides a hint about how the user is currently interacting with the content; normally VIEW,
     * but could be EDIT, HELP, CONFIG, or a custom mode.
     *
     * @since 5.0
     */
    MODE("mode"),
