...
public class PortalApplicationContextLocator implements ServletContextListener {
   ...
   * This was done because Tomcat may instantiate all listeners before calling contextInitialized on
   * any listener.<br>
   ...
   *     use the {@link WebApplicationContextUtils#getRequiredWebApplicationContext(ServletContext)}
   *     directly.
   */
  @Deprecated
   ...
   * not a singleton {@link ApplicationContext} is created if needed and returned. Unless a {@link
   * WebApplicationContext} is specifically needed this method should be used as it will work both
   * when running in and out of a web application
   *
  ...
  public static ApplicationContext getApplicationContext() {
    ...
    if (context != null) {
      ...
      if (applicationContextCreator.isCreated()) {
        ...
        getLogger()
            .error("Stack trace of original ApplicationContext creator", directCreatorThrowable);
