...
public abstract class BaseJpaDao implements InitializingBean, ApplicationContextAware {
  ...
  /**
   * Important common logic for creating and configuring JPA queries cached in EhCache. This step is
   * important for the sake of scalability.
   *
    ...
    if (roots.size() > 1) {
      logger.warn(
          "Query "
              ...
              + " has "
              + roots.size()
              + " roots. The first will be used to generated the cache region name: "
              + cacheRegion);
    }
    return cacheRegion;
  }
