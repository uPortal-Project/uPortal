...
/**
 * This class presents a facade for the IEntityCachingService implementation that lets clients cache
 * and retrieve <code>IBasicEntities</code>. It hides such details as the physical structure of the
 * cache and whether it is running in a multi- or single-JVM environment.
 *
 * <p>An <code>IBasicEntity</code> can answer its type and key.
 *
 * <p>Caching consists of asking the service to add, retrieve, update and remove elements from the
 * cache, e.g.,
 *
 * <p><code>
...
public class EntityCachingService implements IEntityCachingService {
    ...
    private static final SingletonDoubleCheckedCreator<EntityCachingService> instanceHolder =
            new SingletonDoubleCheckedCreator<EntityCachingService>() {
                @Override
    ...
    /**
     * Removes the <code>IBasicEntity</code> from the cache and notifies peer caches.
     *
     * @param ent org.apereo.portal.IBasicEntity
     * @exception org.apereo.portal.concurrency.CachingException
     */
