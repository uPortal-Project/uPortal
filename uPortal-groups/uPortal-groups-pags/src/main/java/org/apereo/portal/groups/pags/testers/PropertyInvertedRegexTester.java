 ...
 * obtained from a property value from portal.properties and returning true for a FAILURE to match.
 * The match function attempts to match the entire region against the pattern specified. <br>
...
public class PropertyInvertedRegexTester extends InvertedRegexTester {
    ...
    private void setPattern(String attribute, String propertyName) {
        String regexExpression = PropertiesManager.getProperty(propertyName, "");
        if (StringUtils.isBlank(regexExpression)) {
            logger.error(
                    "Unable to find property name {} in portal.properties or has empty value."
