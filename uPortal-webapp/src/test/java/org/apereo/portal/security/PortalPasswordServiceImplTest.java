...
public class PortalPasswordServiceImplTest {
    ...
    public void testSHAPassword() {
        ...
        assertTrue(
                "Expected encrypted password to start with '(SHA256)' but was [" + encrypted + "]",
                encrypted.startsWith("(SHA256)"));
    ...
    public void testLegacyPassword() {
        assertTrue(
                passwordService.validatePassword("admin", "(MD5)PfgN2CNzDOPAociIqa31KrWXuxHTgLMp"));
        assertFalse(
                passwordService.validatePassword("admin", "(MD5)PfgN2CNzDOPAociIqa31KrWXuxHTgLMz"));
    }
}
