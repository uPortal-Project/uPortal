...
public class AggregationIntervalHelperImplTest {
        ...
        final List<AcademicTermDetail> terms =
                ImmutableList.<AcademicTermDetail>of(
                        new AcademicTermDetailImpl(
                                new DateMidnight(2012, 1, 12),
                                new DateMidnight(2012, 4, 1),
                                "spring 2"),
                        new AcademicTermDetailImpl(
                                new DateMidnight(2012, 4, 1),
                                new DateMidnight(2012, 7, 1),
                                "summer 2"),
                        new AcademicTermDetailImpl(
                                new DateMidnight(2012, 7, 1),
                        ...
                        new AcademicTermDetailImpl(
                                new DateMidnight(2012, 12, 15),
                                new DateMidnight(2013, 1, 14),
                                "winter 2"),
                        new AcademicTermDetailImpl(
                                new DateMidnight(2015, 12, 15),
                                new DateMidnight(2016, 1, 14),
                                "winter 5"));
    ...
    public void testIntervalsBetweens() {
        ...
        assertEquals(
                1052640,
                helper.getIntervalStartDateTimesBetween(AggregationInterval.MINUTE, start, end)
                        .size());
        assertEquals(
                210528,
                helper.getIntervalStartDateTimesBetween(AggregationInterval.FIVE_MINUTE, start, end)
                        .size());
        assertEquals(
                17544,
                helper.getIntervalStartDateTimesBetween(AggregationInterval.HOUR, start, end)
                        .size());
        assertEquals(
                731,
                helper.getIntervalStartDateTimesBetween(AggregationInterval.DAY, start, end)
                        .size());
        assertEquals(
                104,
                helper.getIntervalStartDateTimesBetween(AggregationInterval.WEEK, start, end)
                        .size());
        assertEquals(
                24,
                helper.getIntervalStartDateTimesBetween(AggregationInterval.MONTH, start, end)
                        .size());
        assertEquals(
                2,
                helper.getIntervalStartDateTimesBetween(AggregationInterval.YEAR, start, end)
                        .size());
        assertEquals(
                3,
                helper.getIntervalStartDateTimesBetween(
                                AggregationInterval.ACADEMIC_TERM, start, end)
                        .size());
        assertEquals(
                8,
                helper.getIntervalStartDateTimesBetween(
                                AggregationInterval.CALENDAR_QUARTER, start, end)
                        .size());
    }
}
