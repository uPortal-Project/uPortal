...
(function ($, fluid) {
  ...
  var deselectEntity = function (that, key) {
    ...
    // Enable submit.
    if (that.options.selected.length === 0 && that.options.requireSelection) {
  ...
  var updateBreadcrumbs = function (
    ...
    if (breadcrumbs.find('span').length > 0) {
      ...
      isKey =
        breadcrumbs.find('span a[key="' + key + '"]').length > 0 ? true : false;
      if (!isKey) {
        buildBreadCrumb(that, key, entity.name, breadcrumbs, browseFunction);
      } // end:if.
    } else {
