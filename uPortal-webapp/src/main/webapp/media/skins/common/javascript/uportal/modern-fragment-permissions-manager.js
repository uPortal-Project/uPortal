...
class ModernFragmentPermissionsManager {
    ...
    initializeDialogs() {
        ...
        const pageDialog = document.querySelector(
            this.options.selectors.pageDialog
        );
        if (pageDialog) {
            this.menus.pagePermissionsManager = new PermissionsMenu(
                pageDialog,
                {
        ...
        const columnDialog = document.querySelector(
            this.options.selectors.columnDialog
        );
        if (columnDialog) {
            this.menus.columnPermissionsManager = new PermissionsMenu(
                columnDialog,
                {
                    savePermissionsUrl: this.options.savePermissionsUrl,
                    elementExtractor: (link) =>
                        link.closest('.portal-page-column'),
                    titleExtractor: (element) => {
                        const columns = document.querySelectorAll(
                            '.portal-page-column'
                        );
                        const index = Array.from(columns).indexOf(element) + 1;
                        return this.options.messages.columnX.replace(
                            '{0}',
                            index
                        );
                    },
        ...
        const portletDialog = document.querySelector(
            this.options.selectors.portletDialog
        );
        if (portletDialog) {
            this.menus.portletPermissionsManager = new PermissionsMenu(
                portletDialog,
                {
                    savePermissionsUrl: this.options.savePermissionsUrl,
                    elementExtractor: (link) =>
                        link.closest('.up-portlet-wrapper'),
                    titleExtractor: (element) => {
                        const titleLink = element.querySelector(
                            '.up-portlet-wrapper-inner h2 a:first-child'
                        );
    ...
    bindEvents() {
        ...
        const pageLink = document.querySelector(
            this.options.selectors.pageDialogLink
        );
        if (pageLink && this.menus.pagePermissionsManager) {
            pageLink.addEventListener('click', (event_) => {
        ...
        document.addEventListener('click', (event_) => {
            const columnLink = event_.target.closest(
                this.options.selectors.columnDialogLink
            );
        ...
        document.addEventListener('click', (event_) => {
            const portletLink = event_.target.closest(
                this.options.selectors.portletDialogLink
            );
...
class PermissionsMenu {
    ...
    refresh(link) {
        const element = this.options.elementExtractor
            ? this.options.elementExtractor(link)
            : null;
    ...
    populateForm(element) {
        ...
        this.setCheckbox(
            form,
            'movable',
            element.classList.contains('movable')
        );
        this.setCheckbox(
            form,
            'deletable',
            element.classList.contains('deletable')
        );
        this.setCheckbox(
            form,
            'editable',
        ...
        this.setCheckbox(
            form,
            'addChildAllowed',
            element.classList.contains('canAddChildren')
        );
