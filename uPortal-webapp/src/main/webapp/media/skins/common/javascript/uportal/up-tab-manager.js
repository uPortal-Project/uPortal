...
(function ($, fluid) {
  ...
  var editTabHandler = function (that) {
    ...
    if (edit.length > 0) {
      ...
      if (numberOfPortlets === 0 && text.text() === that.options.addTabLabel) {
        text.trigger('click');
      } // end:if.
    } // end:if.
  }; // end:function.
  ...
  var moveTabHandler = function (that) {
    ...
    that.reorderLayout = fluid.reorderLayout(that.container, {
      ...
      styles: {
        defaultStyle:
          'fl-reorderer-' + that.options.tabContext + '-movable-default',
        selected:
          'fl-reorderer-' + that.options.tabContext + '-movable-selected',
        dragging:
          'fl-reorderer-' + that.options.tabContext + '-movable-dragging',
        mouseDrag:
          'fl-reorderer-' + that.options.tabContext + '-movable-dragging',
