...
class LayoutPreferences {
    ...
    initializeGallery() {
        if (
            ...
            document.querySelector(this.options.gallerySelector)
        ) {
            this.components.gallery = new window.up.PortalGallery(
                this.options.gallerySelector,
                {
                    isOpen: false,
                }
        ...
        if (tabNavigation) {
            this.components.tabManager = new TabManager('#portalNavigation', {
                ...
                onTabRemove: async (tabContainer) => {
                    if (
                        !(await window.up.confirm(
                            this.options.messages.confirmRemoveTab
                        ))
                    ) {
                        return false;
                    }
                    ...
                    this.persistence.update(
                        ...
                        () => {
                            window.location =
                                this.urlProvider.getPortalHomeUrl();
                        }
                    );
                },
                onTabAdd: (tabLabel, columns, tabGroup) => {
                    this.persistence.update(
                        ...
                        (data) => {
                            const tabUrl = this.urlProvider.getTabUrl(
                                data.tabId
                            );
    ...
    initializePortletReorderer() {
        ...
        if (document.querySelectorAll('[id*=toolbar_]').length > 0) {
            this.components.portletReorderer = new PortletReorderer(
                '#portalPageBodyColumns',
                {
                    ...
                    onAfterMove: (movedNode) => {
                        ...
                        const nextPortlets =
                            movedNode.parentNode.querySelectorAll(
                                '[id^=portlet_]'
                            );
                        const movedIndex =
                            Array.from(nextPortlets).indexOf(movedNode);
                        ...
                        const options = {
                            ...
                            sourceID:
                                window.up.defaultNodeIdExtractor(movedNode),
                        };
                        ...
                        persistence
                            ...
                            .catch(async (error) => {
                                ...
                                if (
                                    await window.up.confirm(
                                        this.options.messages.movePortletError
                                    )
                                ) {
                                    location.reload();
                                }
                        ...
                        const moveOptionsItem = movedNode.querySelector(
                            '.up-portlet-control.move'
                        );
                        if (moveOptionsItem) {
                            moveOptionsItem.textContent =
                                moveOptionsItem.dataset.moveText;
                        }
                        const grabHandle = movedNode.querySelector(
                            '.up-portlet-titlebar .grab-handle'
                        );
                        if (grabHandle) {
    ...
    initializePortletDeletion() {
        // Portlet deletion
        document.addEventListener('click', async (event_) => {
            if (event_.target.matches('a[id*=removePortlet_]')) {
                event_.preventDefault();

                const id = window.up.defaultNodeIdExtractor(event_.target);
                if (
                    !(await window.up.confirm(
                        this.options.messages.confirmRemovePortlet
                    ))
                ) {
                    return false;
                }

                const portletElement = document.querySelector(`#portlet_${id}`);
                const subnavElement = document.querySelector(
                    `#portalSubnavLink_${id}`
                );

                if (portletElement) portletElement.remove();
                if (subnavElement) subnavElement.remove();
                ...
                if (dialog && window.jQuery) {
                    ...
                    if (form) {
                        form.addEventListener('submit', (event_) => {
                            ...
                            const tabId = form.querySelector(
                                '[name=targetTab]:checked'
                            )?.value;
                            ...
                            this.persistence.update(
                                ...
                                () => {
                                    window.location =
                                        this.urlProvider.getTabUrl(tabId);
                                }
                            );
...
class LayoutPreferencesPersistence {
    constructor(container, options = {}) {
        try {
            const modern = new window.ModernLayoutPreferencesPersistence(
                container,
                options
            );
            ...
            console.error(
                'Error during LayoutPreferencesPersistence initialization:',
                error
            );
            throw error;
        }
...
class UrlProvider {
    constructor(container, options = {}) {
        return new window.ModernUrlProvider(container, options);
    }
}
...
class TabManager {
    constructor(selector, options = {}) {
        ...
        return new window.ModernTabManager(selector, options);
    }
}
...
class PortletReorderer {
    ...
    setup() {
        ...
        $(this.options.selectors.columns).droppable({
            ...
            drop: (event, ui) => {
                ...
                if (dropTarget.length > 0) {
                    this.dropped = true;
                    const $element = $(droppedElement).detach();
                    $element.css({
                        position: '',
                        top: '',
                        left: '',
                    ...
                    dropTarget.replaceWith($element);
                    if (this.options.onAfterMove)
                        this.options.onAfterMove(droppedElement);
                }
            },
        });

        $(this.options.selectors.modules + '.movable').on(
            'mousedown',
            function (event_) {
                const r = this.getBoundingClientRect();
                $(this).draggable('option', 'cursorAt', {
                    left: Math.round(event_.clientX - r.left),
                    top: Math.round(event_.clientY - r.top),
                });
            }
        );
        $(this.options.selectors.modules + '.movable').draggable({
            ...
            start: (event, ui) => {
                ...
                this.dropColumn = $(this.draggedEl).closest(
                    this.options.selectors.columns
                )[0];
                ui.helper.css({
                    zIndex: 9999,
                    width: $(this.draggedEl).outerWidth(),
                });
                $(this.dropTargetMarkup).insertBefore(this.draggedEl);
                const $next = $(this.draggedEl)
                    .nextAll(this.options.selectors.portlet)
                    .first();
                this.dropInsertBefore = $next.length > 0 ? $next[0] : null;
                this.mouseMoveHandler = (event_) => this.onMouseMove(event_);
            ...
            stop: () => {
                ...
                if (!this.dropped) {
                    const initialTarget = $(
                        this.options.selectors.pseudoDropTarget
                    ).first();
                    if (initialTarget.length > 0) {
                        const $element = $(this.draggedEl).detach();
                        $element.css({
                            position: '',
                            top: '',
                            left: '',
                        ...
                        initialTarget.replaceWith($element);
                    }
                }
                this.draggedEl = null;
    ...
    onMouseMove(event_) {
        ...
        const portlets = targetColumn
            .find(this.options.selectors.portlet)
            .not(this.draggedEl);
        let insertBefore = null;
        portlets.each(function () {
            const rect = this.getBoundingClientRect();
            if (event_.clientY < rect.top + rect.height / 2) {
                insertBefore = this;
                return false;
            }
        });
