...
(function ($, fluid) {
  up.TranslatorPortlet = function (container, options) {
    ...
    var portletFormSubmitHandler = function () {
      ...
      $.ajax({
        ...
        success: function () {
          ...
          $message.html(that.options.messages.portletTranslationSaved);
          $message.show();
    ...
    var refreshEntities = function (data) {
      ...
      var entities = [];
      $(data.entities.sort(up.getStringPropertySortFunction('title'))).each(
      ...
      if (entityTemplate == null) {
        entityTemplate = fluid.selfRender(that.locate('entities'), entities, {
          cutpoints: selectorMap
        });
      } else {
        fluid.reRender(entityTemplate, that.locate('entities'), entities, {
          cutpoints: selectorMap
        });
      }
    };
      ...
      $.ajax({
        ...
        success: function () {
          ...
          var $message = that
            .locate('formContainer')
            .find('.portlet-msg-success');
          $message.html(that.options.messages.messageTranslationSaved);
          $message.show();
          $message.fadeOut(4000);
        }
      });
