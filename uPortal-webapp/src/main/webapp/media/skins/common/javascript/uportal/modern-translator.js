...
class ModernTranslatorPortlet {
    ...
    bindElements() {
        this.entityTypeSelect = this.container.querySelector(
            this.options.selectors.entityType
        );
        this.localeSelect = this.container.querySelector(
            this.options.selectors.locale
        );
        this.entitiesContainer = this.container.querySelector(
            this.options.selectors.entities
        );
        this.entityList = this.container.querySelector(
            this.options.selectors.entityList
        );
        this.portletForm = this.container.querySelector(
            this.options.selectors.portletForm
        );
        this.messageForm = this.container.querySelector(
            this.options.selectors.messageForm
        );
        this.formContainer = this.container.querySelector(
            this.options.selectors.formContainer
        );
        this.addMessageBtn = this.container.querySelector(
            this.options.selectors.addMessage
        );
    }
    ...
    bindEvents() {
        ...
        this.portletForm.addEventListener('submit', (event_) =>
            this.submitPortletForm(event_)
        );
        this.messageForm.addEventListener('submit', (event_) =>
            this.submitMessageForm(event_)
        );
        this.addMessageBtn.addEventListener('click', () => this.addMessage());

        // Prevent entity form submission
        const entityForm = this.container.querySelector(
            this.options.selectors.entityForm
        );
        if (entityForm) {
            entityForm.addEventListener('submit', (event_) => {
    ...
    refreshEntities(data) {
        ...
        this.addMessageBtn.style.display =
            this.options.entityType === 'message' ? 'block' : 'none';
        ...
        const sortedEntities = data.entities.sort(
            up.getStringPropertySortFunction('title')
        );
        ...
        for (const entity of sortedEntities) {
            ...
            button.addEventListener('keydown', (event_) => {
                if (event_.key === 'Enter') {
                    // Enter key
                    selectEntity();
                }
            ...
            this.entitiesContainer.append(button);
        }
    }
    ...
    async fillPortletForm() {
        try {
            ...
            this.setFormValue(
                '.uptrans-pfrm-orig-title',
                portlet.original.title
            );
            this.setFormValue(
                '.uptrans-pfrm-loc-title',
                portlet.localized.title
            );
            ...
            this.setFormValue(
                '.uptrans-pfrm-orig-descr',
                portlet.original.description
            );
            this.setFormValue(
                '.uptrans-pfrm-loc-descr',
                portlet.localized.description
            );
    ...
    async fillMessageForm() {
        try {
            ...
            this.messageForm.querySelector(
                '.uptrans-mfrm-code-fieldset'
            ).style.display = 'none';
        } catch (error) {
    ...
    async submitPortletForm(event_) {
        event_.preventDefault();
        ...
        try {
            ...
            if (response.ok) {
                this.showSuccessMessage(
                    this.options.messages.portletTranslationSaved
                );
            }
        } catch (error) {
    ...
    async submitMessageForm(event_) {
        event_.preventDefault();
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.showSuccessMessage(
                    this.options.messages.messageTranslationSaved
                );
            }
        } catch (error) {
    ...
    showSuccessMessage(message) {
        const messageElement = this.formContainer.querySelector(
            '.portlet-msg-success'
        );
        if (messageElement) {
            messageElement.innerHTML = message; // Use innerHTML like original
