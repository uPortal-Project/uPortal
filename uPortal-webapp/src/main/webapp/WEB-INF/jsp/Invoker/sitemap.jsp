...
<template id="sitemap-tab-template">
	<div id="sitemap-tab-header" class="col-md-3">
		<h4>
			<a href=""></a>
		</h4>
		<ul></ul>
...
<script
	src="<rs:resourceURL value="/rs/promise-polyfill/6.0.2/promise.js"/>"></script>
...
<%--
    UI strings for i18n
--%>
<spring:message var="i18n_error_loading_sitemap"
	code="error.loading.sitemap" />
...
fetch('${layoutApiUrl}', {credentials: 'same-origin'})
    ...
    .then(function (response) {
		...
		var tabRow = document.importNode(tabRowTemplate.content, true);
		var tabCounter = 0;
		_.forEach(response.layout.navigation.tabs, function (tab, tabIndex) {
			...
			var tabTemplate = document.getElementById('sitemap-tab-template');
           	...
           	tabHeaderLink.href = '/uPortal/f/' + tab.ID + '/normal/render.uP';
			...
			_.forEach(tab.content, function (parentContent, parentContentIndex) {
				...
				_.forEach(parentContent.content, function (portlet, portletIndex) {
					...
					var portletTemplate = document.getElementById('sitemap-tab-portlet-template');
           			var portletListItem = document.importNode(portletTemplate.content, true);
					...
					var portletLink = portletListItem.querySelector('a');
           			portletLink.href = '/uPortal/f/' + tab.ID + '/p/'+portlet.fname+'.'+portlet.ID+'/max/render.uP';
			...
			if(tabIndex == (response.layout.globals.tabsInTabGroup-1)) {		
				...
				document.getElementById('sitemap-holder').appendChild(tabRow);
				tabRow = document.importNode(tabRowTemplate.content, true);
			}
		});
    })
    ...
    .catch(function(error) {
		...
		var errDiv = document.importNode(errTemplate.content, true);
