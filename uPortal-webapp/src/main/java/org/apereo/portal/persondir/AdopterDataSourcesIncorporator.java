...
public class AdopterDataSourcesIncorporator {
     ...
     * We need to find beans that implement <code>IPersonAttributeDao</code> defined by adopters and
     * add them to this list (at the end). Finally, we need to provide the complete list to the
     * <code>mergingPersonAttributeDao</code>.
     */
    ...
    public void init() {
        ...
        final Map<String, IPersonAttributeDao> allDaos =
                applicationContext.getBeansOfType(IPersonAttributeDao.class);
        for (Map.Entry<String, IPersonAttributeDao> y : allDaos.entrySet()) {
            logger.debug(
                    "Evaluating IPersonAttributeDao bean with id='{}' to see if it's uPortal "
                            + "internal or adopter-defined",
                    y.getKey());
            if (!rootPersonAttributeDao.equals(y.getValue())
                    && !uPortalInternalPersonAttributeDaos.contains(y.getValue())) {
                logger.info(
                        "Identified adopter-defined IPersonAttributeDao bean with id='{}';  this "
                                + "bean will be added to Person Directory configuration",
                        y.getKey());
         ...
         */
        final List<IPersonAttributeDao> finalList =
                new ArrayList<>(innerMergedPersonAttributeDaoList);
