...
public class ServletAnnotationMappingUtilsTest {
    ...
    public void checkSpringMultipleHeadersTest() {
        final String[] headers = {
            "org.apereo.portal.url.UrlType=RENDER", "org.apereo.portal.url.UrlState!=EXCLUSIVE"
        };
            ...
            for (String header : headers) {
                ...
                if (separator == -1) {
                    ...
                    if (isMediaTypeHeader(key)) {
                        List<MediaType> requestMediaTypes =
                                MediaType.parseMediaTypes(request.getHeader(key));
                        List<MediaType> valueMediaTypes = MediaType.parseMediaTypes(value);
                        boolean found = false;
                        for (Iterator<MediaType> valIter = valueMediaTypes.iterator();
                                valIter.hasNext() && !found;
                                ) {
