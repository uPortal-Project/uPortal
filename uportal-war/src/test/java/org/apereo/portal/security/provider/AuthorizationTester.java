...
public class AuthorizationTester extends TestCase {
      ...
      if ((s_factoryName = pr.getProperty("authorizationProvider")) == null) {
        ...
        try {
          IAuthorizationServiceFactory factory =
              (IAuthorizationServiceFactory) Class.forName(s_factoryName).newInstance();
  ...
  protected void tearDown() {
    try {
      IPermission[] permissions =
          (IPermission[]) testPermissions.toArray(new IPermission[testPermissions.size()]);
  ...
  public void testAlternativePermissionPolicies() throws Exception {
    ...
    print(msg);
    testResult =
        prin.hasPermission(OWNER, activity, nonExistingTarget, getPositivePermissionPolicy());
    assertTrue(msg, testResult);

    msg = "Testing NEGATIVE permission policy for a nonexistent channel";
    print(msg);
    testResult =
        prin.hasPermission(OWNER, activity, nonExistingTarget, getNegativePermissionPolicy());
    assertTrue(msg, !testResult);
   ...
   * when the property org.apereo.portal.security.IAuthorizationService.cachePermissions=true, since
   * performance of the db calls will distort the time needed to complete the various parts of the
   * test.
   */
