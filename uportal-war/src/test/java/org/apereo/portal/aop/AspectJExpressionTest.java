...
public class AspectJExpressionTest {
  @Autowired private TestAspect testAspect;

  @Autowired private RepositoryPointcutInterface repositoryPointcutInterface;

  @Autowired private CountingMethodInterceptor countingMethodInterceptor;
  ...
  public void testProgramaticPointcut() {
    final RepositoryPointcutInterface targetPointcutInterface =
        new RepositoryPointcutInterfaceImpl();

    final AspectJProxyFactory portletPreferencesProxyFactory =
        new AspectJProxyFactory(targetPointcutInterface);

    final Method interceptorMethod =
        ReflectionUtils.findMethod(
            CountingMethodInterceptor.class, "countInvocation", ProceedingJoinPoint.class);
    final AspectJAroundAdvice aspectJAroundAdvice =
    ...
    final RepositoryPointcutInterface proxiedPointcutInterface =
        (RepositoryPointcutInterface) portletPreferencesProxyFactory.getProxy();
