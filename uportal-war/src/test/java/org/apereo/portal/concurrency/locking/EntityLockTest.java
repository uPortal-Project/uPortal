...
public class EntityLockTest extends TestCase {
  ...
  protected void setUp() {
    try {
      ...
      try {
        ...
        lockStore =
            (multiServer) ? RDBMEntityLockStore.singleton() : MemoryEntityLockStore.singleton();
      } catch (Exception e) {
        System.out.println(
            "EntityLockTester:setUp(): Failed to instantiate entity lock store. " + e);
      }

      lockStore.deleteAll();

      java.util.Date earlier = new java.util.Date(System.currentTimeMillis() - 1000); // - 1 second
      java.util.Date later = new java.util.Date(System.currentTimeMillis() + 300000); // + 5 minutes
  ...
  protected void tearDown() {
    try {
      ...
      for (int i = 0; i < testLocks.length; i++) {
        getLockStore().delete(testLocks[i]);
      }
    } catch (Exception ex) {
  ...
  public void testServiceConvert() throws Exception {
    ...
    IEntityLock lock =
        service.newLock(
            GROUP_CLASS, testKeys[3], IEntityLockService.READ_LOCK, testIds[0], readSecs);
    ...
    try {
      duplicateLock =
          service.newLock(GROUP_CLASS, testKeys[3], IEntityLockService.WRITE_LOCK, testIds[1]);
    } catch (LockingException le) {
      print("Caught exception: " + le.getMessage());
    }
  ...
  public void testServiceNewLock() throws Exception {
    ...
    IEntityLockService service = getService();
    IEntityLock newLock =
        service.newLock(GROUP_CLASS, testKeys[3], IEntityLockService.WRITE_LOCK, testIds[0]);
    msg = "Testing if new lock is valid";
  ...
  public void testStoreUpdate() throws Exception {
    ...
    IEntityLock lock1 =
        new EntityLockImpl(
            testLocks[1].getEntityType(),
            testLocks[1].getEntityKey(),
            testLocks[1].getLockType(),
            testLocks[1].getExpirationTime(),
            testLocks[1].getLockOwner());
