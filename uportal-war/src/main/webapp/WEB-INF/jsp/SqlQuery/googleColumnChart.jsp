...
<script type="text/javascript">
  ...
  function drawChart() {
    ...
    <c:forEach items="${ results[0] }" var="cell" varStatus="status">
        data.addColumn('${ status.index == 0 ? "string" : "number" }', '<spring:escapeBody javaScriptEscape="true">${ cell.key }</spring:escapeBody>');
    </c:forEach>
    ...
    <c:forEach items="${ results }" var="row" varStatus="status">
        <c:forEach items="${ row }" var="cell" varStatus="cellStatus">
            <c:choose><c:when test="${ cellStatus.index == 0 }">
                data.setValue(${ status.index }, ${ cellStatus.index }, '<spring:escapeBody javaScriptEscape="true">${ cell.value }</spring:escapeBody>');
            </c:when><c:otherwise>
