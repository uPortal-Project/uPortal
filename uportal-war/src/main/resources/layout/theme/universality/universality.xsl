...
<xsl:stylesheet 
  ...
  <xsl:param name="UP_VERSION">uMobile</xsl:param>
    ...
    <xsl:if test="$INSTITUTION='uportal' or $INSTITUTION='coal' or $INSTITUTION='ivy' or $INSTITUTION='hc'">
      ...
      <!-- Footer Links -->
	      ...
	      <div id="portalProductAndVersion">
	        <p>
                <a href="http://www.jasig.org/umobile" title="Powered by uMobile ${UP_VERSION}" target="_blank">Powered by uMobile <xsl:value-of select="$UP_VERSION"/></a>, an open-source project by <a href="http://www.jasig.org" title="Jasig.org - Open for Higher Education">Jasig</a> - <span><xsl:value-of select="$SERVER_NAME"/></span>
                <xsl:if test="$AUTHENTICATED='true'">
                    <br/>
