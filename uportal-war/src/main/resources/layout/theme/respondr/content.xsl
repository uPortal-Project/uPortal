  ...
  <xsl:template name="controls">
    ...
    -->
      ...
      <xsl:variable name="configurable">
          <xsl:if test="parameter[@name='configurable'] and parameter[@name='configurable']/@value = 'true' and upAuth:hasPermission('UP_PORTLET_PUBLISH', 'PORTLET_MODE_CONFIG', 'PORTLET_ID.@ID')">true</xsl:if>
      </xsl:variable>
      ...
      <xsl:if test="$configurable='true'">
        ...
        <li>
          <a href="{portletConfigureUrl}#{@ID}" title="{upMsg:getMessage('configure.portlet', $USER_LANG)}" class="up-portlet-control configure"><xsl:value-of select="upMsg:getMessage('configure', $USER_LANG)"/></a>
        </li>
      </xsl:if>
