                ...
                <xsl:if test="@type='regular' or @type='favorite_collection'">
                    ...
                    <xsl:for-each select="channel">
                        <li>
                            ...
                            <xsl:variable name="portletUrl">{up-portlet-link(<xsl:value-of select="@ID" />,<xsl:value-of
                                 select="$defaultPortletUrl" />)}</xsl:variable>
                            <xsl:variable name="iconUrl">
                                <xsl:choose>
                                    <xsl:when
                                         test="parameter[@name='mobileIconUrl'] and parameter[@name='mobileIconUrl']/@value != ''">
                                        <xsl:value-of select="parameter[@name='mobileIconUrl']/@value"/>
                                    </xsl:when>
                                    <xsl:otherwise><xsl:value-of select="$CONTEXT_PATH"/>/media/skins/icons/mobile/default.png</xsl:otherwise>
                                </xsl:choose>
                            </xsl:variable>
                            <xsl:variable name="newItemCountClasses">ui-li-count badge new-item
                             up-new-item-count-{up-portlet-new-item-count(<xsl:value-of select="@ID" />)}</xsl:variable>
                            <xsl:element name="a">
                              <xsl:attribute name="href"><xsl:value-of select="$portletUrl" /></xsl:attribute>
                              <xsl:attribute name="title"><xsl:value-of select="upMsg:getMessage('to.view', $USER_LANG)"
                              ...
                              <span class="{$newItemCountClasses}">{up-portlet-new-item-count(<xsl:value-of select="@ID"
                                   />)}</span>
                            </xsl:element>
                        </li>
                    </xsl:for-each>
