...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xmsg="urn:x-lexica:xmsg:message:1.0">
  ...
  <xsl:template match="/">
		...
		function grpDeleteGroup(path){
			if (window.confirm('Sei sicuro di voler eliminare definitivamente questo gruppo, i suoi permessi ed i suoi membri??')){
				this.location.href=path;
			}
		}
		</SCRIPT>
      ...
      <xsl:if test="not($mode='edit') and not($mode='members')">
        ...
        <form action="{$baseActionURL}" method="POST">
				...
				<xsl:if test="$highlightedGroup[not(@id=0) and not(@searchResults='true')]"> 
					...
					<em>ricerca solo negli appartenenti a questo gruppo</em>
					</td>
					</tr>
				</xsl:if>
			</form>
				  ...
				  <td colspan="5">
					...
					<xsl:if test="not($blockFinishActions)">
						<xsl:if test="count(descendant::*[@selected='true'])">
						  ...
						  <input type="submit" class="uportal-button"  onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Done';" value="Fatto (selezionati)" />
						</xsl:if>
					  <xsl:text>
					  </xsl:text>
					  <input type="submit" class="uportal-button"  onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Cancel';" value="Elimina selezionati" />
					</xsl:if>
				  </td>
				</tr>
            </xsl:if>
      ...
      <xsl:if test="not($mode='members')">
          	<xsl:choose>
          		...
          		<xsl:when test="$group/@searchResults='true'">
          			<a href="javascript:this.location.href='{$baseActionURL}?grpCommand=Delete&amp;grpCommandArg={$group/@id}';"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/delete.gif" alt="Elimina Gruppo" title="Elimina Gruppo"/></a>
          		</xsl:when>
          		<xsl:when test="$mode='edit'">
          			<a href="{$baseActionURL}?grpCommand=Unlock&amp;grpCommandArg={$group/@id}"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/unlock.gif" alt="Fine modifica Gruppo" title="Fine modifica Gruppo"/></a>
          			<xsl:if test="$group/@canDelete='true'">
						<a href="javascript:grpDeleteGroup('{$baseActionURL}?grpCommand=Delete&amp;grpCommandArg={$group/@id}');"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/delete.gif" alt="Elimina Gruppo" title="Elimina Gruppo"/></a>
					</xsl:if>
          		</xsl:when>
          		<xsl:when test="($group/@editable='true') and ($group/@canUpdate='true' or $group/@canAssignPermissions='true' or $group/@canManageMembers='true' or $group/@canCreateGroup='true')">
          			<a href="{$baseActionURL}?grpCommand=Lock&amp;grpCommandArg={$group/@id}"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/lock.gif" alt="Modifica Gruppo" title="Modifica Gruppo"/></a>
          		</xsl:when>
          	</xsl:choose>
          	
          	<a href="{$baseActionURL}?grpCommand=Highlight&amp;grpCommandArg="><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/close.gif" alt="Chiudi Gruppo" title="Chiudi Gruppo"/></a>
      </xsl:if>
          </td>
  </xsl:template>
  ...
  <xsl:template name="rightPane">
				...
				<td nowrap="nowrap" colspan="2">
                  <xsl:if test="$mode='edit'">
					<xsl:if test="$group/@canUpdate='true'">
						<input type="submit" onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Update';" value="Aggiorna" class="uportal-button" />
					...
					<xsl:if test="not($highlightedGroupID='0') and not($grpKey='null') and $group/@canAssignPermissions='true'">
						<input type="submit"  onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Permissions';" value="Assegna i Permessi" class="uportal-button" />
					  </xsl:if>
                  </xsl:if>
					<xsl:if test="$group/@canManageMembers='true' or ($grpServantMode='true')">
						<input type="submit"  onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Add';" value="Aggiungi membro" class="uportal-button" />
					</xsl:if>
				</td>
			</tr>
              <xsl:if test="$mode='edit'">
			  ...
			  <xsl:if test="$group/@canCreateGroup='true'">
				  <tr>
					...
					<td colspan="2" nowrap="true">
					  <input type="submit" onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Create';" value="Crea nuovo Gruppo" class="uportal-button" />
					  <xsl:text>
					  </xsl:text>
    ...
    <xsl:if test="$mode='select'">
            <tr>
              <td colspan="2">
                <input type="submit" onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Select';"  value="Seleziona i contrassegnati" class="uportal-button" />
      ...
      <xsl:if test="(($mode='edit') or ($mode='members')) and ($grpServantMode='true')">
		...
		<input type="hidden" name="grpCommand" value="Cancel" />
		  <tr>
			...
			<td colspan="2">
			  <input type="submit" value="Finito" class="uportal-button" />
			</td>
		  </tr>
		</form>
	</xsl:if>
  ...
  <xsl:template name="propertiesDisplay">
    ...
    <tr><td></td>
        ...
        <xsl:choose>
                ...
                <xsl:otherwise>
                        <em>Nessuna ulteriore informazione disponibile</em>
                </xsl:otherwise>
        </xsl:choose>
        </td>
    </tr>
