...
public class HeaderHandlingDispatcherPortlet extends DispatcherPortlet {
  ...
  protected void doHeaders(RenderRequest request, RenderResponse response) {
    try {
      ...
      logger.error(
          "Exception rendering headers for portlet " + getPortletName() + ". Aborting doHeaders",
          ex);
    }
  }
   ...
   * HandlerAdapter will be obtained by querying the portlet's installed HandlerAdapters to find the
   ...
   *   <li>Though a streaming portlet likely will invoke RENDER_HEADERS before RENDER_MARKUP, there
   *       is no guarantee of order of execution; e.g. render_markup may be called before
   ...
   * exception for the RENDER_MARKUP phase) and probably render no output if there was an exception.
   * However triggerAfterRenderCompletion in the superclass is a private method so it cannot be
   * executed in a subclass. For now we can only have RENDER_HEADERS perform the normal processing
   * until Spring Framework Portlet MVC code incorporates this behavior.
   *
