...
public class CachingResourceLoaderImpl implements CachingResourceLoader {
   ...
   */
  @Override
  public <T> CachedResource<T> getResource(Resource resource, Loader<T> builder, long checkInterval)
      throws IOException {
    ...
    if (lastCheckTime + checkInterval >= System.currentTimeMillis()) {
      if (this.logger.isTraceEnabled()) {
        this.logger.trace(
            cachedResource + " is within checkInterval " + checkInterval + ", returning");
      }
  ...
  protected <T> boolean checkIfModified(CachedResource<T> cachedResource) {
    ...
    if (resourceModified) {
      this.logger.trace(
          "Resource {} was modified at {}, reloading", new Object[] {resource, mainLastModified});
      return true;
    }
    ...
    this.logger.trace(
        "{} has not been modified since last loaded {}, returning", cachedResource, lastLoadTime);
    return false;
  }
