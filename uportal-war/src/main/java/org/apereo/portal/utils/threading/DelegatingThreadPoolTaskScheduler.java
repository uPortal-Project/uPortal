...
public class DelegatingThreadPoolTaskScheduler extends ThreadPoolTaskScheduler
  ...
  protected long getAdditionalStartDelay() {
    ...
    if (this.lastStartDelay != 0) {
      this.lastStartDelay =
          Math.max(0, this.initialDelay - (System.currentTimeMillis() - this.initialized));
  ...
  protected Date getDelayedStartDate(Date startDate) {
    ...
    if (additionalStartDelay > 0) {
      ...
      logger.debug(
          "Updated startDate with additionalStartDelay from " + startDate + " to " + newStartDate);
      return newStartDate;
    }
  ...
  private static class DelegatingForwardingScheduledFuture<V> extends DelegatingForwardingFuture<V>
