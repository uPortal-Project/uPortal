...
public class Util {
  ...
  /**
   * URL encode a path segment. This is just a thin wrapper around UriUtils.encodePathSegment. It is
   * intended for the case where you are building URLs in JS. c:url + escapeBody doesn't correctly
   * escape the contents (especially "</script>"), and fn:escapeXml incorrectly encodes the URL (it
   * escapes chars like '<' as &lt; instead of %3C). It should help avoid XSS attacks when building
   * RESTful URLS in js. Example:
   *
   ...
   * possible to end up with REST URLs that won't work correctly (like the one above), but at least
   * this will protect you from XSS attacks on the front end.
   *
