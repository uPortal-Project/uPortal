...
public final class PortalWebUtils {
   ...
   *
   * <p>The request attributes mutex is guaranteed to be the same object during the entire lifetime
   * of the request, available under the key defined by the <code>REQUEST_MUTEX_ATTRIBUTE</code>
  ...
  /**
   * Get a {@link ConcurrentMap} for the specified name from the {@link ServletRequest} attributes.
   * If it doesn't exist create it and store it in the attributes. This is done in a thread-safe
   * matter that ensures only one Map per name & request will be created @See {@link
  ...
  /**
   * Get a {@link ConcurrentMap} for the specified name from the {@link HttpSession} attributes. If
   * it doesn't exist create it and store it in the attributes. This is done in a thread-safe matter
   * that ensures only one Map per name & session will be created @See {@link
   ...
   * Requires that Spring have bound the request, as in the case of dispatcher servlet or portlet or
   * when the binding filter or listener is active. This should be the case for all requests in the
   * uPortal framework and framework portlets.
   *
