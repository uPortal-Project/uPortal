...
public class FragmentActivator {
  ...
  public void setUserViews(
      @Qualifier("org.apereo.portal.layout.dlm.FragmentActivator.userViews") Ehcache userViews) {
    this.userViews =
    ...
    if (owner.getAttribute(NEWLY_CREATED_ATTR) != null) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Failed to save layout for newly created fragment owner " + owner.getUserName(), e);
      }
    }
  ...
  private void fragmentizeLayout(UserView view, FragmentDefinition fragment) {
    ...
    boolean allowExpandedContent =
        Boolean.parseBoolean(PropertiesManager.getProperty(PROPERTY_ALLOW_EXPANDED_CONTENT));
  ...
  /**
   * Recursive method that passes through a layout tree and changes all ids from the regular format
   * of sXX or nXX to the globally safe incorporated id of form uXlXsXX or uXlXnXX indicating the
   * user id and layout id from which this node came.
   */
