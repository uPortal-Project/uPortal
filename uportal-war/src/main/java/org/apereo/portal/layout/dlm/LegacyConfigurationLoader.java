...
/**
 * Implementation of {@link ConfigurationLoader} that behaves exactly as DLM has always done: load
 * {@link FragmentDefinition} objects from dlm.xml.
 *
 * @author awills
 * @deprecated
 */
public class LegacyConfigurationLoader implements ConfigurationLoader {
  ...
  private final SingletonDoubleCheckedCreator<Boolean> loadedFlag =
      new SingletonDoubleCheckedCreator<Boolean>() {
  ...
  protected void loadFragmentInfo() {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could not open InputStream to dlm configuration resource " + this.configurationFile, e);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could not convert dlm configuration resource to URL " + this.configurationFile, e);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could load dlm configuration resource " + this.configurationFile, e);
    } catch (SAXException e) {
      throw new IllegalArgumentException(
          "Could parse dlm configuration resource " + this.configurationFile, e);
    }
    ...
    if (localFragments != null) {
      ...
      final Map<String, FragmentDefinition> fragmentsByOwnerId =
          new LinkedHashMap<String, FragmentDefinition>();
      for (final FragmentDefinition fragmentDefinition : localFragments) {
  ...
  /* (non-Javadoc)
   * @see org.apereo.portal.layout.dlm.ConfigurationLoader#getFragments()
   */
  @Override
  ...
  protected List<FragmentDefinition> getFragments(NodeList frags) {
    ...
    for (int i = 0; i < fragmentCount; i++) {
      ...
      try {
        ...
        final String msg =
            "Unable to load distributed layout fragment definition. Content from this fragment will not be avilable.";
        if (this.logger.isDebugEnabled()) {
          logger.debug(msg + XmlUtilitiesImpl.toString(fragmentElement), e);
        } else {
          logger.warn(
              msg
                  + " Enable DEBUG logging for stack trace.\n\tCaused By: "
                  + e.getMessage()
                  + " "
                  + XmlUtilitiesImpl.toString(fragmentElement));
        }
      }
    }
