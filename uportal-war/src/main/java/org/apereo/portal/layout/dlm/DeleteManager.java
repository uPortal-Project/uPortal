...
public class DeleteManager {
   ...
   * aquiring a single instance of the configured layout store so we delegate over there so that all
   * references refer to the same instance. This method is solely for convenience so that we don't
   * have to keep calling UserLayoutStoreFactory and casting the resulting class.
   */
  ...
  /**
   * Attempt to apply a single delete command and return true if it succeeds or false otherwise. If
   * the delete is disallowed or the target element no longer exists in the document the delete
   ...
   * If this node contains any incorporated elements then they must also have a delete directive
   * added in here to prevent incorporated channels originating in another column from reappearing
   * in that column because the position set entry that pulled them into this column was now
   * removed. (ie: the user moved an inc'd channel to this column and then deleted the column means
   * that the inc'd channel should be deleted also.) This was designed to add a delete directive for
   * each nested element having an ID so as to work for the future case of a tree view.
   */
