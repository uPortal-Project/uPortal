...
public class ParameterEditManager {
  ...
  private static boolean applyEdit(Element edit, Document ilf) {
    ...
    if (override != null && !override.getNodeValue().equals(Constants.CAN_OVERRIDE)) return false;
  ...
  /**
   * Get the parameter edits set if any stored in the root of the document or create it if passed-in
   * create flag is true.
   */
   ...
   * values to a named parameter of an incorporated channel represented by the passed-in target id.
   * If one doesn't exists for that node and that name then this call returns without any effects.
   */
