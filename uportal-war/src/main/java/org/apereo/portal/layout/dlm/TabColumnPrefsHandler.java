...
public class TabColumnPrefsHandler {
   ...
   * moved but need a separate handle of the parent from whence it came. The goal of this method is
   * to make the appropriate change in the PLF to persist this action take by the user. For ILF
   * nodes this generally means adding an entry to the position set for the new parent and removing
   * any entry if it existed in the position set in the old parent. For nodes that are owned by the
   * user (PLF owned nodes) the nodes are moved outright and now position set is needed unless the
   * new parent contains ILF nodes as well requiring preservation of the user's ordering of the
   * nodes for when the ILF and PLF are merged again later on.
   */
  public static void moveElement(Element compViewNode, Element oldCompViewParent, IPerson person)
    ...
    if (compViewNode.getAttribute(Constants.ATT_ID).startsWith(Constants.FRAGMENT_ID_USER_PREFIX)) {
      ...
      if (LOG.isInfoEnabled()) LOG.info("ilf node being moved, only update new parent pos set");
      PositionManager.updatePositionSet(compViewParent, plfParent, person);
    } else {
      // plf node
      if (LOG.isInfoEnabled()) LOG.info("plf node being moved, updating old parent's position set");
