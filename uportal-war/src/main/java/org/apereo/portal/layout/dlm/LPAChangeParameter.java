...
public class LPAChangeParameter implements ILayoutProcessingAction {
  ...
  LPAChangeParameter(String nodeId, String name, String value, IPerson p, Element ilfNode) {
    this.nodeId = nodeId;
    this.name = name;
    this.person = p;
  ...
  /**
   * Change the parameter for a channel in both the ILF and PLF using the appropriate mechanisms for
  ...
  /**
   * Look in the passed-in element for a parameter child element whose value is to be changed to the
   * passed-in value.
   *
