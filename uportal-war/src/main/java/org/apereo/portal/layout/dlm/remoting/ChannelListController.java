...
public class ChannelListController {
  ...
  /**
   * Updated version of this API. Supports an optional 'categoryId' parameter. If provided, this URL
   * will return the portlet registry beginning with the specified category, including all
  ...
  private Map<String, SortedSet<?>> getRegistryOriginal(WebRequest request, IPerson user) {
    ...
    categories.add(
        prepareCategoryBean(request, rootCategory, portletsNotYetCategorized, user, locale));
  ...
  private ChannelCategoryBean prepareCategoryBean(
    ...
    for (PortletCategory childCategory :
      ...
      ChannelCategoryBean childCategoryBean =
          prepareCategoryBean(request, childCategory, portletsNotYetCategorized, user, locale);
  ...
  private Map<String, SortedSet<?>> getRegistry43(
    ...
    final Locale locale = getUserLocale(user);
    categories.add(
        preparePortletCategoryBean(request, rootCategory, portletsNotYetCategorized, user, locale));
    ...
    if (includeUncategorized) {
      ...
      final String uncDescription =
          messageSource.getMessage(UNCATEGORIZED_DESC, new Object[] {}, locale);
      PortletCategory pc =
          new PortletCategory(uncName); // Use of this String for Id matches earlier version of API
      pc.setName(uncName);
  ...
  private PortletCategoryBean preparePortletCategoryBean(
    ...
    for (PortletCategory childCategory :
      ...
      PortletCategoryBean childBean =
          preparePortletCategoryBean(req, childCategory, portletsNotYetCategorized, user, locale);
    ...
    PortletCategoryBean categoryBean =
        PortletCategoryBean.fromPortletCategory(category, subcategories, marketplacePortlets);
    categoryBean.setName(messageSource.getMessage(category.getName(), new Object[] {}, locale));
   ...
   * ${request.contextPath} for the iconUrl parameter, presumably because uPortal can be deployed to
   * a context other than /uPortal. We should either...
   *
   * <p>- Discontinue SpEL in publishing parameters entirely; or - Extend it to parameters beyond
   * 'iconUrl'
   *
   * <p>And if we continue using SpEL in parameters, we should evaluate it when they're read out of
   * the database (long before now).
   *
  ...
  private IPortletDefinitionParameter postProcessIconUrlParameter(
    ...
    if (!ICON_URL_PARAMETER_NAME.equals(iconUrl.getName())) {
      String msg =
          "Only iconUrl should be processed this way;  parameter was:  " + iconUrl.getName();
