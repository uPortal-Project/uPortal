...
public final class NodeReferenceFactory {
  ...
  public Noderef getNoderefFromPathref(
    ...
    if (pathTokens[0].equals(layoutOwner)) {
      ...
      this.log.warn(
          "Unable to resolve pathref '" + pathref + "' for layoutOwner '" + layoutOwner + "'");
      return null;
    }
    ...
    if (structIdAttr != null) {
      ...
      if (isStructRef) {
        return new Noderef(
            layoutOwnerUserId, 1 /* TODO:  remove hard-coded layoutId=1 */, "s" + structId);
      }
    ...
    return new Noderef(
        layoutOwnerUserId, 1 /* TODO:  remove hard-coded layoutId=1 */, idAttr.getTextContent());
  }
   ...
   * owner's layout, together with the username. This version of the layout consistent with what DLM
   * uses internally for fragments, and is created by FragmentActivator.fragmentizeLayout. It's
   * important that the version returned by this method matches what DLM uses internally because it
   * will be used to establish relationships between fragment layout nodes and user customizations
   * of DLM fragments.
   *
   * @param userId
   * @return
