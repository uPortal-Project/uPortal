...
public class UserPrefsHandler {
   ...
   * preferences object and not part of the layout. These attributes are specified in the .sdf files
   * for the structure and theme stylesheets. The value is not stored in the layout but the loading
   * of user prefs joins to a layout struct and hence that struct must exist in the layout. This
   * call gets that node into the PLF if not there already and prevents it from being removed if no
   * other changes were made to it or its children by the user.
   */
