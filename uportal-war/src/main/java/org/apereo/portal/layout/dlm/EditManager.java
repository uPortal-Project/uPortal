...
public class EditManager {
   ...
   * attribute was changed and the value in the plf copy node should be used, if allowed, during the
   * merge at login time.
   */
  ...
  private static void addDirective(
      Element plfNode, String attributeName, String type, IPerson person) throws PortalException {
  ...
  /**
   * Evaluate whether attribute changes exist in the ilfChild and if so apply them. Returns true if
   * some changes existed. If changes existed but matched those in the original node then they are
   * not applicable, are removed from the editSet, and false is returned.
   */
  ...
  /**
   * Searches for a dlm:pref command which indicates that a user preference was change and if found
   * removes it from the user's PLF.
   */
  ...
  /**
   * Searches for a dlm:edit command which indicates that a node attribute was reset to the value in
   * the fragment and if found removes it from the user's PLF.
   */
