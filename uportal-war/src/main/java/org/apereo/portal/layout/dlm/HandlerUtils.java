...
public class HandlerUtils {
   ...
   * node with the same id is not found in the PLF then null is returned. If the create is true then
   * an attempt is made to create the node along with any necessary ancestor nodes needed to
  ...
  public static Element createPlfNodeAndPath(
      Element compViewNode, boolean includeChildNodes, IPerson person) throws PortalException {
    ...
    if (compViewNode.getAttribute(Constants.ATT_ID).startsWith(Constants.FRAGMENT_ID_USER_PREFIX))
      return createILFCopy(compViewNode, compViewParent, includeChildNodes, plf, plfParent, person);
  ...
  /**
   * Creates or moves the plf copy of a node in the composite view and inserting it before its next
   * highest sibling so that if dlm is not used then the model ends up exactly like the original
   * non-dlm persistance version. The position set is also updated and if no ilf copy nodes are
  ...
  static Element createOrMovePLFOwnedNode(
    ...
    while (child != null) {
      if (insertionPointFound
          ...
          && !child.getAttribute(Constants.ATT_ID).startsWith(Constants.FRAGMENT_ID_USER_PREFIX))
