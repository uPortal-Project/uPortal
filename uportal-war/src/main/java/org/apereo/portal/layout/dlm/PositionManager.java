...
public class PositionManager {
  ...
  /**
   * This method and ones that it delegates to have the responsibility of organizing the child nodes
   * of the passed in composite view parent node according to the order specified in the passed in
   * position set and return via the passed in result set whether the personal layout fragment (one
   * portion of which is the position set) or the incoporated layouts fragment (one portion of which
   * is the compViewParent) were changed.
   *
   ...
   * or PLF; the UI elements that they own. These node do not exist in their layout in the database
   * but instead are merged in with their owned elements at log in and other times. So to move them
   * during subsequent merges a position set can contain a position directive indicating the id of
   * the node to be moved into a specific position in the sibling list and that well may refer to a
   * node not in the sibling list to begin with. If the node no longer exists in the composite view
   * then that position directive can safely be discarded.
   *
   ...
   *
   * <p>1) nodes with moveAllowed="false" prevent nodes of lower precedence from being to their left
   * or higher with left or higher defined as having a lower index in the sibling list.
   * (applyLowerPrecRestriction)
   *
   * <p>2) nodes with moveAllowed="false" prevent nodes of equal precedence from moving from one
   * side of this node to the other from their position as found in the compViewParent initially and
   * prevents nodes with the same precedence from moving from other parents into this parent prior
   * to the restricted node. Prior to implies a lower sibling index. (applyHoppingRestriction)
   *
   ...
   * last ordered them) should maintain their relative order as much as possible and be appended to
   * the end of the sibling list after all others rules have been applied. (getOrder)
   *
   * <p>Each of these rules is applied by a call to a method 5 being first and 1 being last so that
   * 1 has the highest precedence and last say. Once the final ordering is specified then it is
   * applied to the children of the compViewParent and returned.
   */
  ...
  /**
   * This method is responsible for preventing nodes with lower precedence from being located to the
   * left (lower sibling order) of nodes having a higher precedence and moveAllowed="false".
   */
   ...
   * from hopping over each other so that a layout fragment can lock two tabs that are next to each
   * other and they can only be separated by tabs with higher precedence.
   *
  ...
  static boolean isIllegalHoppingSpecified(List<NodeInfo> order) {
    for (int i = 0; i < order.size(); i++) {
      ...
      for (int j = i + 1; j < order.size(); j++) {
        ...
        if (niSib.getIndexInCVP() == -1 || niSib.getPrecedence() == Precedence.getUserPrecedence())
          continue;
   ...
   * if there are any down- stream (higher sibling index) siblings that have moveAllowed="false". If
   * any such sibling is found then the node is not allowed to be reparented and is removed from the
   * list.
   */
  ...
  /**
   * Return true if the passed in node or any downstream (higher index) siblings <strong>relative to
   * its destination location</strong> have moveAllowed="false".
   */
  private static boolean isNotReparentable(
    ...
    try {
      ...
      for (Element nextPlaceholder =
          ...
          nextPlaceholder =
              (Element) nextPlaceholder.getNextSibling()) { // Advance to the next placeholder
        ...
        final NodeList unmaskPlaceholderList =
            (NodeList) unmaskPlaceholder.evaluate(compViewParent, XPathConstants.NODESET);
        switch (unmaskPlaceholderList.getLength()) {
          ...
          case 1:
            ...
            if (nextSibling.getAttribute(Constants.ATT_MOVE_ALLOWED).equals("false")) {
               ...
               */
              Precedence p =
                  Precedence.newInstance(nextSibling.getAttribute(Constants.ATT_FRAGMENT));
  ...
  /**
   * This method assembles in the passed in order object a list of NodeInfo objects ordered first by
   * those specified in the position set and whose nodes still exist in the composite view and then
   * by any remaining children in the compViewParent.
   */
  static void applyOrdering(
      List<NodeInfo> order, Element compViewParent, Element positionSet, NodeInfoTracker tracker) {
  ...
  /**
   * This method locates the position set element in the child list of the passed in plfParent or if
   * not found it will create one automatically and return it if the passed in create flag is true.
   */
  ...
  /**
   * Create, append to the passed in position set, and return a position element that references the
   * passed in elementID.
   */
