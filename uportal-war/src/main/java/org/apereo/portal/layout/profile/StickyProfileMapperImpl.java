...
public class StickyProfileMapperImpl
  ...
  /**
   * Log a warning when configured such that a profile key both means apathy and means a particular
   * profile fname.
   */
  @PostConstruct
  ...
  public String getProfileFname(IPerson person, HttpServletRequest request) {
    ...
    try {
      ...
      logger.error(
          "Failed to read persisted profile selection for user " + userName + " if any.  Ignoring.",
          e);
    }
   ...
   *
   * @param profileKeyForNoSelection potentially null key that you want to mean apathy about profile
   *     selection
   */
