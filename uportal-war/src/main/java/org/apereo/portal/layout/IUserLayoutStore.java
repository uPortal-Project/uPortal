...
public interface IUserLayoutStore {
  ...
  /**
   * Returns an <code>Element</code> representing the user's layout and <code>UserPreferences</code>
   * (but not portlet preferences) formatted for export. This element <em>must</em> have an element
   * name of &lt;layout&gt;. Exported documents <em>must not</em> reference database identifiers and
   * <em>should</em> exclude unnecessary items like channel publishing parameters, etc. Layout store
   * implementations are <em>may</em> return <code>null</code> for users that don't have layout or
  ...
  /**
   * Performs the reverse of <code>exportLayout</code>. The specified element <em>must</em> have an
   * element name of &lt;layout&gt; and <emshould</em> contain both content and <code>
   ...
   *
   * @param person User
   * @return a <code>Hashtable</code> mapping user profile ids (<code>Integer</code> objects) to the
   *     {@link UserProfile} objects
   */
  ...
  /**
   * Returns an object suitable for identifying attribute values for folder nodes and attribute and
   * parameter values for channel nodes in a user's layout that differ from the values on the same
   * element in a fragment. This is used by the layout manager to know which ones must be persisted.
   *
   * @param sId
   ...
   * Precedence is actually based on two elements in a fragment definition: the precedence and the
   * index of the fragment definition in the dlm.xml file. If two fragments are given equal
   * precedence then the index if relied upon to resolve conflicts with UI elements.
   */
