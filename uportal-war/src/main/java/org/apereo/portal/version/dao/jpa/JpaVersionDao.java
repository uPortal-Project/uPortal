...
public class JpaVersionDao extends BasePortalJpaDao implements VersionDao {
            ...
            new Function<CriteriaBuilder, CriteriaQuery<Integer>>() {
              ...
              public CriteriaQuery<Integer> apply(CriteriaBuilder cb) {
                ...
                criteriaQuery.select(
                    versionRoot.get(VersionImpl_.local).alias(VersionImpl_.local.getName()));
                criteriaQuery.where(
                    cb.equal(versionRoot.get(VersionImpl_.product), productParameter));
  ...
  /**
   * Load a Version object with direct field queries. Used to deal with DB upgrades where not all of
   * the fields have been loaded
   */
  private Version getSimpleVersion(String product) {
    ...
    final Integer major =
        coreNumbers.get(VersionImpl_.major.getName(), VersionImpl_.major.getBindableJavaType());
    final Integer minor =
        coreNumbers.get(VersionImpl_.minor.getName(), VersionImpl_.minor.getBindableJavaType());
    final Integer patch =
        coreNumbers.get(VersionImpl_.patch.getName(), VersionImpl_.patch.getBindableJavaType());
  ...
  public Version setVersion(String product, Version version) {
    return this.setVersion(
        product, version.getMajor(), version.getMinor(), version.getPatch(), version.getLocal());
  }
}
