...
public abstract class AbstractPersonManager implements IPersonManager {
   ...
   * org.apereo.portal.security.PersonFactory.guest_user_names</code> property in portal.properties
   * and (optionally) any beans that implement {@link IGuestUsernameSelector}. This approach
   * supports pluggable, open-ended strategies for multiple guest users who may have different
   * content.
   *
   * @since 5.0
   */
  ...
  private synchronized void loadGuestUserId(String username, Map<String, Integer> map) {
    ...
    if (userId == null) {
      final String msg =
          "The specified guest user account does not exist in the portal database:  " + username;
      throw new IllegalStateException(msg);
    }
