...
public class SimpleLdapSecurityContext extends ChainingSecurityContext
  ...
  public synchronized void authenticate() throws PortalSecurityException {
    ...
    if (propFile != null && propFile.length() > 0) ldapConn = LdapServices.getLdapServer(propFile);
      ...
      try {
        ...
        if (conn != null) {
          try {
            ...
            log.info(
                "SimpleLdapSecurityContext: Password invalid for user: " + this.myPrincipal.UID);
          } catch (Exception e) {
            log.error(
                "SimpleLdapSecurityContext: LDAP Error with user: " + this.myPrincipal.UID + "; ",
                e);
  ...
  /**
   * Return a single value of an attribute from possibly multiple values, grossly ignoring anything
   * else. If there are no values, then return an empty string.
   *
