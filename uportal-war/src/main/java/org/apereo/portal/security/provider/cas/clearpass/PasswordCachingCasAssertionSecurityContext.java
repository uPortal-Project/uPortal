...
public class PasswordCachingCasAssertionSecurityContext extends CasAssertionSecurityContext {
  ...
  /**
   * Attempt to use the PGTIOU returned in the service ticket validation to obtain a proxy ticket to
   * call CAS to get the user's password. The Proxy Granting Ticket IOU (PGTIOU) from the service
   * ticket validation response has already been matched to the Proxy Granting Ticket (PGT) that CAS
   * delivers to one of the uPortal servers and the PGT should be present in the assertion. If
   * password retrieval fails, log messages to help the user identify the issue. It may be that the
   * uPortal servers are in a cluster and the configuration is not correct so this uPortal server
   * does not have the PGT.
   *
  ...
  protected final String retrievePasswordUsingProxyTicket(final String proxyTicket) {
    try {
      ...
      if (log.isWarnEnabled()) {
        log.warn(
            "Unable to retrieve the credential from the ClearPass " + "service for proxy ticket {}",
            proxyTicket,
            e);
      }
      return null;
