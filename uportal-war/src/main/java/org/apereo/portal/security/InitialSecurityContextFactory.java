...
public class InitialSecurityContextFactory {
  ...
  /**
   * Recursively parses the tree of {@link ContextConfiguration} objects to create a tree (chain) of
   * {@link ISecurityContext}s. The root context is returned by the method after all of it's
   ...
   * parsed to find all context configuration properties and sub-contexts for this context. For each
   * sub-context this method is called recursively.
   *
  ...
  private static ContextConfiguration loadContextConfigurationChain(
    ...
    if (lastDotIndex >= 0) {
      try {
        ...
        final PortalSecurityException pse =
            new PortalSecurityException("Invalid context name " + fullContextName, ioobe);
    ...
    contextConfig.subConfigs =
        (ContextConfiguration[]) subContexts.toArray(new ContextConfiguration[subContexts.size()]);
