...
public class AnyUnblockedGrantPermissionPolicy implements IPermissionPolicy {
  ...
  public boolean doesPrincipalHavePermission(
    ...
    if (!activity.equals(allPermissionsActivity)) { // NOTE:  Must check to avoid infinite recursion
      ...
      if (doesPrincipalHavePermission(
          service, principal, allPermissionsOwner, allPermissionsActivity, allPermissionsTarget)) {
    ...
    try {
      ...
      rslt =
          hasUnblockedPathToGrantWithCache(service, principal, owner, activity, target, seenGroups);
    } catch (Exception e) {
  ...
  public void loadInCache(
    ...
    final CacheTuple cacheTuple =
        new CacheTuple(
            principal.getPrincipalString(), owner.getFname(), activity.getFname(), target.getKey());
   ...
   * responsible for performing the same check for affiliated groups in the Groups hierarchy, but it
   * is NOT responsible for understanding the nuances of relationships some activities and/or
   ...
   *   <li>Find out what groups this principal belongs to; convert each one to a principal and seek
   *       an answer by invoking ourselves recursively; if an answer is found in this step, return
   *       it
