...
public class LogoutController implements InitializingBean {
   ...
   * is returned for the first authenticated context that has an associated redirect string. If such
   * a context is not found, we use the default DEFAULT_REDIRECT that was originally setup.
   *
   * <p>NOTE: This will work or not work based upon the logic in the root context. At this time, all
   * known security contexts extend the ChainingSecurityContext class. If a context has the variable
   * stopWhenAuthenticated set to false, the user may be logged into multiple security contexts. If
   * this is the case, the logout process currently implemented does not accommodate multiple
   * logouts. As a reference implemention, the current implementation assumes only one security
   * context has been authenticated. Modifications to perform multiple logouts should be considered
   * when a concrete need arises and can be handled by this class or through a change in the
        ...
        while (subCtxNames.hasMoreElements()) {
          ...
          if (sc.isAuthenticated()) {
            ...
            if (redirect != null && !redirect.equals("")) {
              if (log.isDebugEnabled()) {
                log.debug(
                    "LogoutController::getRedirectionUrl()" + " subCtxName redirect = " + redirect);
              }
              break;
            }
          }
