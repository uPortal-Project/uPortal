...
public final class RequestParameterPolicyEnforcementFilter implements Filter {
   ...
   *
   * <p>If the String is null, return the empty set. If the whitespace delimited String contains no
   * tokens, throw IllegalArgumentException. If the sole token is an asterisk, return the empty set.
  ...
  static Set<String> parseParametersToCheck(final String initParamValue) {
    ...
    if (0 == tokens.length) {
      throw new IllegalArgumentException(
          "[" + initParamValue + "] had no tokens but should have had at least one token.");
    }
    ...
    for (final String parameterName : tokens) {
      ...
      if ("*".equals(parameterName)) {
        throw new IllegalArgumentException(
            "Star token encountered among other tokens in parsing [" + initParamValue + "]");
      }
   ...
   *
   * <p>If the String is null (init param was not set) default to DEFAULT_CHARACTERS_BLOCKED. If the
   * String is "none" parse to empty set meaning block no characters. If the String is empty throw,
   * to avoid configurer accidentally configuring not to block any characters.
   *
