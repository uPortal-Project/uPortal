...
public class ReferenceEntityLockService implements IEntityLockService {
  ...
  private void initialize() throws LockingException {
    ...
    if (multiServer) {
      try {
        int lockTolerance =
            PropertiesManager.getPropertyAsInt("org.apereo.portal.concurrency.clockTolerance");
   ...
   *     lock we check the store a second time and roll back if any other lock has snuck in. I think
   *     this is slightly safer than depending on the db isolation level for transactional
   *     integrity.
   */
   ...
   * Param <code>lockType</code> can be null.
   *
   * @param entityType
   * @param entityKey
   * @param lockType (optional)
