...
public class ClusterLockServiceImpl implements IClusterLockService {
  ...
  public <T> TryLockFunctionResult<T> doInTryLock(
      final String mutexName, Function<ClusterMutex, T> lockFunction) throws InterruptedException {
  ...
  public <T> TryLockFunctionResult<T> doInTryLock(
    ...
    try {
      ...
      if (lastRunDelay > 0 || serverBiasDelay > 0) {
        ...
        if (lastRunDelay > 0) {
          ...
          if (clusterMutex.getLockStart() > nextRunTime
            ...
            this.logger.trace(
                "db lock last run less than {}ms ago for {}", lastRunDelay, mutexName);
      ...
      if (mutex == null) {
        ...
        this.logger.trace(
            "failed to aquire database lock, returning notExecuted result for: {}", mutexName);
        return TryLockFunctionResultImpl.getSkippedInstance(LockStatus.SKIPPED_LOCKED);
      }
