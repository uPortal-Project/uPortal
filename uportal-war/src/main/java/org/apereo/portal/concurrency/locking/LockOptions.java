...
public final class LockOptions {
  ...
  /**
   * If set to a value greater than 0 the service will not attempt to acquire the lock if it was
   * last held within the last lastRunDelay milliseconds. <br>
  ...
  /**
   * If set to a value greater than 0 the service will not attempt to acquire the lock if it was
   * last held by a different server within the last serverBiasDelay milliseconds. <br>
   * Useful for biasing work that happens within a lock to run on the same server on each successive
   * run. <br>
