...
public interface IClusterLockService {
  ...
  interface TryLockFunctionResult<T> {
    ...
    /**
     * @return The function result, if {@link #isExecuted()} returns false this method should be
     *     ignored
     */
    T getResult();
  }
  ...
  enum LockStatus {
    ...
    EXECUTED,
    /**
     * {@link LockOptions#getLastRunDelay()} was greater than 0 and the lock was last held less than
     * that many milliseconds ago
     */
    SKIPPED_LAST_RUN,
    /**
     * {@link LockOptions#getServerBiasDelay()} was greater than 0 and the lock was last held by a
     * different server less than that many milliseconds ago
     */
    SKIPPED_SERVER_BIAS,
  ...
  <T> TryLockFunctionResult<T> doInTryLock(String mutexName, Function<ClusterMutex, T> lockFunction)
      throws InterruptedException;

  /**
   * Execute the specified function within the named mutex. If the mutex is currently owned by
   * another thread or server the method will return immediately.
   *
   * @param mutexName Name of the lock (case sensitive)
   * @param lockOptions Options when getting the lock
   * @param lockFunction The fuction to call within the lock context, the parameter to the function
   *     is the locked {@link ClusterMutex}
   * @return The value returned by the lockFunction
   */
  <T> TryLockFunctionResult<T> doInTryLock(
