...
public class PortletWindowAttributeSource implements AttributeSource, BeanNameAware {
    ...
    public final Iterator<Attribute> getAdditionalAttributes(
        ...
        final Tuple<IPortletWindow, StartElement> portletWindowAndElement =
                this.portletWindowRegistry.getPortletWindow(request, event);
        ...
        for (final IPortletWindow portletWindow : portletWindows) {
            if (portletWindow != null) {
                ...
                cacheKeyBuilder.addAll(
                        portletWindowId, windowState.toString(), portletMode.toString());
            } else {
                this.logger.warn(
     ...
     *
     * <p>Extension point. Used by the FixedWindowStateAttributeSource to render a theme specific
