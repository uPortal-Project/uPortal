...
public abstract class SkinMappingTransformerConfigurationSource
  ...
  /**
   * If true the result of {@link #getSkinName(HttpServletRequest)} will be cached in the user's
   * session and re-used. If false {@link #getSkinName(HttpServletRequest)} will be called on every
   * execution.
   */
  ...
  private String getSkinNameInternal(HttpServletRequest request) {
    if (!this.cacheSkinResolution
        || Included.PLAIN == this.resourcesElementsProvider.getDefaultIncludedType()) {
  ...
  protected boolean shouldOverrideSkin(HttpServletRequest request) {
    ...
    final IStylesheetDescriptor stylesheetDescriptor =
        stylesheetUserPreferencesService.getStylesheetDescriptor(request, PreferencesScope.THEME);
