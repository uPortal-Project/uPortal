...
public class RDBMUserIdentityStore implements IUserIdentityStore {
        ...
        new TransactionCallbackWithoutResult() {
          ...
          protected void doInTransactionWithoutResult(TransactionStatus arg0) {
            ...
            final List<? extends IStylesheetUserPreferences> stylesheetUserPreferences =
                stylesheetUserPreferencesDao.getStylesheetUserPreferencesForUser(userId);
   ...
   *
   * @param person
   * @param createPortalData indicating whether to try to create all uPortal data for this user from
   *     template prototype
            ...
            try {
              ...
              try {
                if (log.isDebugEnabled())
                  log.debug(
                      "RDBMUserIdentityStore::getPortalUID(userName=" + userName + "): " + query);
          ...
          public Boolean doInConnection(Connection con) throws SQLException, DataAccessException {
            ...
            try {
              ...
              try {
                if (log.isDebugEnabled())
                  log.debug(
                      "RDBMUserIdentityStore::getTemplateUser(userId=" + userId + "): " + query);
                rs = pstmt.executeQuery();
                if (rs.next()) {
                  ...
                  public Object doInConnection(Connection con)
                    ...
                    try {
                      ...
                      try {
                        ...
                        if (RDBMServices.getDbMetaData().supportsTransactions()) con.commit();
  ...
  protected int addNewUser(final int newUID, final IPerson person, final TemplateUser templateUser)
    ...
    IGroupMember template =
        GroupService.getEntity(
            templateUser.getUserName(), Class.forName("org.apereo.portal.security.IPerson"));
    for (IEntityGroup eg : template.getParentGroups()) {
      ILockableEntityGroup leg = getSafeLockableGroup(eg, me);
      if (leg != null) {
        addPersonToGroup(person, me, leg);
