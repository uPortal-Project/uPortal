...
/**
 * This helper class processes HttpServletRequests for parameters relating to user layout actions,
 * propagating appropriate events to the user's layout manager, preferences, and channel manager.
 *
 * <p>This class results from factoring the processUserLayoutParameters method out of UserInstance
 * in an effort to make UserInstance smaller and more literate.
 */
...
public class UserLayoutParameterProcessor implements IRequestParameterProcessor {
  ...
  public boolean processParameters(HttpServletRequest request, HttpServletResponse response) {
    ...
    switch (urlState) {
      case DETACHED:
        this.stylesheetUserPreferencesService.setStylesheetParameter(
            request, PreferencesScope.STRUCTURE, "detached", Boolean.TRUE.toString());
      case MAX:
        {
          ...
          if (portletRequestInfo != null) {
            ...
            final IPortletWindow portletWindow =
                this.portletWindowRegistry.getPortletWindow(request, targetWindowId);
            final IPortletEntity portletEntity = portletWindow.getPortletEntity();

            final String channelSubscribeId = portletEntity.getLayoutNodeId();

            this.stylesheetUserPreferencesService.setStylesheetParameter(
                request, PreferencesScope.STRUCTURE, "userLayoutRoot", channelSubscribeId);
      ...
      default:
        {
          this.stylesheetUserPreferencesService.setStylesheetParameter(
              request, PreferencesScope.STRUCTURE, "userLayoutRoot", IUserLayout.ROOT_NODE_NAME);
          break;
        }
    }
  ...
  protected String findTabIndex(IUserLayoutManager userLayoutManager, String tabId) {
    ...
    int tabIndex = 0;
    for (String topNodeId = rootsChildren.nextElement();
        rootsChildren.hasMoreElements();
