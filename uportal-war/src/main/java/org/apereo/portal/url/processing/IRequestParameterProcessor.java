...
public interface IRequestParameterProcessor {
   ...
   * appropriate components. This method can also add, modify and remove parameters on the request.
   * If the request is not yet in a state where it can be completely processed this method may
   * return <code>false</code> so that it can be called again after other processors have been
   * allowed to execute. Even if <code>false</code> is returned as much processing as possible
