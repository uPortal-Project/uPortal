...
public class PortalPermissionEvaluator implements PermissionEvaluator {
  ...
  public boolean hasPermission(
    ...
    // translate it into a permission relevant to the provided target
      ...
      throw new RuntimeException(
          "Unable to determine permission target id for type " + targetDomainObject.getClass());
    }

    if (activity != null) {
      final boolean hasPermission =
          principal.hasPermission(activity.getOwnerFname(), activity.getActivityFname(), targetId);
    ...
    if (activity != null) {
      final boolean hasPermission =
          principal.hasPermission(
              activity.getOwnerFname(), activity.getActivityFname(), targetId.toString());
      return hasPermission;
    } else {
      return false;
    }
  ...
  private AuthorizableActivity getViewActivity(
    ...
    if (entity != null && activityKey.equals("VIEW")) {
      ...
      if (type.isGroup()) {
        return new AuthorizableActivity(IPermission.PORTAL_GROUPS, IPermission.VIEW_GROUP_ACTIVITY);
      } else if (type.equals(EntityEnum.PERSON)) {
        return new AuthorizableActivity(IPermission.PORTAL_USERS, IPermission.VIEW_USER_ACTIVITY);
