...
public class PortalPreAuthenticatedProcessingFilter
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
          "STARTING [" + uuid.toString() + "] for URI=" + httpr.getRequestURI() + " #milestone");
    }
    ...
    /**
     * Override the base class's main filter method to bypass this filter if we're currently at the
     * login servlet. Since that servlet sets up the user session and authentication, we need it to
     * run before this filter is useful.
     */
  ...
  /**
   * Helper inner class for encapsulating logic for determining whether or not the request is for an
   * identity "swap" or "unswap", and determining the "swap from" and "swap to" values.
   */
  ...
  private void publishProfileSelectionEvent(final ProfileSelectionEvent event) {
    try {
      ...
      logger.error(
          "Exception on firing profile selection event " + event, exceptionFiringProfileSelection);
    }
  }
  ...
  /**
   * Get the values represented by each token from the request and load them into a HashMap that is
   * returned.
   *
   * @param tokens
   * @param request
   ...
   * this class implementation of setApplicationEventPublisher (which this class had to override in
   * its parent class because that parent class failed to expose a getter method like this!)
   *
