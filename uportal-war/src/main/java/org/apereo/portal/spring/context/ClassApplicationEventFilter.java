...
/**
 * Extension of SmartApplicationListener that allows configuration of a set of supported and ignored
 * event classes.
 *
 * <p>extend GenericApplicationListenerAdapter
 */
public class ClassApplicationEventFilter<E extends ApplicationEvent>
        implements ApplicationEventFilter<E>, InitializingBean {
    protected final Logger logger = LoggerFactory.getLogger(this.getClass());

    private final Map<Class<? extends ApplicationEvent>, Boolean> supportedEventsCache =
    ...
    /**
     * If no <code>supportedEvents</code> {@link Collection} is configured all {@link
     * ApplicationEvent} sub-classes are supported otherwise matching is done. The property defaults
     * to null (all event types)
     *
    ...
    public void setSupportedEvents(Collection<Class<? extends ApplicationEvent>> supportedEvents) {
        ...
        if (supportedEvents == null) {
            ...
            this.supportedEvents =
                    new LinkedHashSet<Class<? extends ApplicationEvent>>(supportedEvents);
        }
    }

    /**
     * If no <code>ignoredEvents</code> {@link Collection} is configured all {@link
     * ApplicationEvent} sub-classes are supported otherwise matching is done. The property defaults
     * to null (all event types)
     *
     * @param supportedEvents the supportedEvents to set
    ...
    public void setIgnoredEvents(Collection<Class<? extends ApplicationEvent>> ignoredEvents) {
        ...
        if (ignoredEvents == null) {
            ...
            this.ignoredEvents =
                    new LinkedHashSet<Class<? extends ApplicationEvent>>(ignoredEvents);
        }
    }
            ...
            if (targetClass != this.getClass()) {
                typeArg =
                        GenericTypeResolver.resolveTypeArgument(
                                targetClass, ApplicationListener.class);
            }
        }
    ...
    /* (non-Javadoc)
     * @see org.apereo.portal.spring.context.ApplicationEventFilter#supports(org.springframework.context.ApplicationEvent)
     */
    @Override
