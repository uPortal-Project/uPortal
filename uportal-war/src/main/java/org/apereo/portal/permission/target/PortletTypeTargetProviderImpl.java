...
public class PortletTypeTargetProviderImpl implements IPermissionTargetProvider {
  ...
  /**
   * This is an example of a "collective target." It represents all instances of a certain class of
   * thing (portlet types, in this case). NOTE: There are other collective targets; e.g.
  ...
  public static final IPermissionTarget ALL_PORTLET_TYPES_TARGET =
      new PermissionTargetImpl(
          IPermission.ALL_PORTLET_TYPES, IPermission.ALL_PORTLET_TYPES, TargetType.PORTLET_TYPE);
    ...
    if (key.equals(IPermission.ALL_PORTLET_TYPES)) {
      ...
      for (Map.Entry<IPortletType, PortletPublishingDefinition> y : cpds.entrySet()) {
        if (y.getKey().getName().equals(key)) {
          rslt =
              new PermissionTargetImpl(
                  y.getKey().getName(), y.getKey().getName(), TargetType.PORTLET_TYPE);
        }
      }
    }
