...
public class PortletsRESTController {
  ...
  /**
   * Provides information about all portlets in the portlet registry. NOTE: The response is governed
   * by the <code>IPermission.PORTLET_MANAGER_xyz</code> series of permissions. The actual level of
   * permission required is based on the current lifecycle state of the portlet.
   */
  ...
  public @ResponseBody String getRenderedPortlet(
    ...
    try {
      ...
      final String rslt =
          portletExecutionManager.getPortletOutput(portletWindow.getPortletWindowId(), req, res);
      return rslt;
    } catch (Exception e) {
