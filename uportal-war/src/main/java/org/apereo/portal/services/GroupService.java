...
public final class GroupService implements IGroupConstants {
  ...
  /**
   * Returns a pre-existing <code>IEntityGroup</code> or null if the <code>IGroupMember</code> does
   * not exist.
   *
  ...
  /**
   * Receives notice that the UserInstance has been unbound from the HttpSession. In response, we
   * remove the corresponding group member from the cache. We use the roundabout route of creating a
   * group member and then getting its EntityIdentifier because we need the EntityIdentifier for the
   * group member, which is cached, not the EntityIdentifier for the IPerson, which is not.
   *
   * @param person org.apereo.portal.security.IPerson
   */
  ...
  private IEntityGroup igetDistinguishedGroup(String name) throws GroupsException {
    try {
      ...
      throw new GroupsException(
          "GroupService.getDistinguishedGroup(): " + "could not find key for: " + name, ex);
    }
  }
  ...
  public static EntityIdentifier[] searchForEntities(String query, int method, Class<?> type)
      throws GroupsException {
    LOGGER.trace(
        "Invoking searchForEntities for query='{}', method='{}', type='{}'", query, method, type);
