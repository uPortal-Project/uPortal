...
public class ActivityController {
  ...
  public ModelAndView summary(PortletRequest request) throws TypeMismatchException {
    ...
    final Boolean showSearches =
        Boolean.valueOf(prefs.getValue(PREFERENCE_SHOW_SEACHES, DEFAULT_PREFERENCE_SHOW_SEARCHES));
    if (showSearches) {
  ...
  private PortalActivity buildPortalActivity(PortletRequest request, int timeframe) {
    ...
    List<String> displayGroups =
        Arrays.asList(
            prefs.getValues(PREFERENCE_DISPLAY_GROUPS, DEFAULT_PREFERENCE_DISPLAY_GROUPS));
    boolean displayOther =
        Boolean.valueOf(prefs.getValue(PREFERENCE_DISPLAY_OTHER, DEFAULT_PREFERENCE_DISPLAY_OTHER));
    int masterTotal = 0;
    int absTotal = 0;
    int subTotal = 0;
