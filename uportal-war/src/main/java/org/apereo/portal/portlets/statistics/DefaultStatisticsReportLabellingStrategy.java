...
public class DefaultStatisticsReportLabellingStrategy
  ...
  /**
   * Create report title. Criteria that have a single value selected are put into the title. If only
   * 1 of each item chosen, the 1st item will be a column header and the remaining items will be in
   * the title header. Format and possible options for title are:
   *
   * <ul>
   *   <li>itemX
   *   <li>itemX - itemY [ - itemZ...]
   *   <li>item2 [ - item 3...] if all items have items.count = 1
   * </ul>
   *
   * @return report title
   */
  ...
  /**
   * Create column descriptions for the portlet report. The column descriptions are essentially the
   * opposite of the title description changes. Those items that have size > 1 (more than one value
   * selected in the report criteria) are displayed in the column description. If all items have
   * only 1 value selected, the first item will be the column description.
   *
   * @param items ordered array of items in the report. NOTE: item ordering must be the same as with
