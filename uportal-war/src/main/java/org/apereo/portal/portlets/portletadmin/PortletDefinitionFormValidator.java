...
public class PortletDefinitionFormValidator {
    ...
    switch (selectedTypeId) {
      ...
      default:
        ...
        final PortletPublishingDefinition cpd =
            channelPublishingDefinitionDao.getChannelPublishingDefinition(selectedTypeId);
        if (cpd == null) {
          context.addMessage(
              new MessageBuilder()
                  .error()
  ...
  private void doBasicInfo(PortletDefinitionForm def, MessageContext context) {
    ...
    if (StringUtils.isEmpty(def.getFname())) {
      context.addMessage(
          new MessageBuilder().error().source("fName").code("please.enter.fname").build());
      ...
      context.addMessage(new MessageBuilder().error().source("fName").code("fname.in.use").build());
    }

    // Title
    if (StringUtils.isEmpty(def.getTitle())) {
      context.addMessage(
          new MessageBuilder().error().source("title").code("please.enter.title").build());
    }
    ...
    if (def.getId() == null
      ...
      context.addMessage(new MessageBuilder().error().source("name").code("name.in.use").build());
    }
  }
