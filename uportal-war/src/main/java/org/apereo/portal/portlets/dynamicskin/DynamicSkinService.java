...
public interface DynamicSkinService {
  ...
  /**
   * Create the skin css file in a thread-safe manner that allows multiple different skin files to
   * be created simultaneously to handle large tenant situations where all the custom CSS files were
   * cleared away after a uPortal deploy.
   *
   * <p>Since the less compilation phase is fairly slow (several seconds) and intensive, this method
   * will allow multiple threads to process different less compilations at the same time but ensure
   * the same output file will not be created multiple times. Also this method will not let a bad
   * LESS file cause repeated LESS compilations and completely take down the portal. The bad file
   * will be blacklisted for a period of time to limit performance impacts.
   *
   ...
   */
  void generateSkinCssFile(
      PortletRequest request, String filePathname, String skinToken, String lessfileBaseName);

  /**
   * Return a String hashcode of the portlet preference values in a repeatable fashion by
   * calculating them based on sorted portlet preference names. Though hashcode does not guarantee
   * uniqueness, from a practical perspective we'll have so few different values we can reasonably
   * assume preference value combinations will be unique.
   *
   * <p>This calculation process must duplicate computeDefaultHashcode.
   *
   * @param request Portlet request
