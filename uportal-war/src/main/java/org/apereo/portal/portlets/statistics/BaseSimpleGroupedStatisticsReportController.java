...
public abstract class BaseSimpleGroupedStatisticsReportController<
  ...
  /**
   * Default implementation to create a map of the report column discriminators based on the
  ...
  protected Map<D, SortedSet<T>> getDefaultGroupedColumnDiscriminatorMap(F form) {
    ...
    for (final Long queryGroupId : groups) {
      final D groupMapping =
          createGroupedDiscriminatorInstance(this.aggregatedGroupDao.getGroupMapping(queryGroupId));
