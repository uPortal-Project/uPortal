...
public class SitemapPortletController {
   ...
   * document doesn't contain required attributes for tab group functionality. Those attributes must
   * be incorporated into user's layout using this component.
   */
  ...
  /**
   * Auto-wires in the PortalRequestUtils when available. Annotated as not required because portlet
   * detects and treats as a runtime (rather than component-initialization-time) error failure to
   * find dependencies.
   *
  ...
  public ModelAndView displaySitemap(PortletRequest request) throws XMLStreamException {
    ...
    final PipelineEventReader<XMLEventReader, XMLEvent> reader =
        attributeIncorporationComponent.getEventReader(httpServletRequest, httpServletResponse);
