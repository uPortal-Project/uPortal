...
import org.apereo.portal.events.aggr.login.LoginAggregationDiscriminator;
import org.apereo.portal.events.aggr.login.LoginAggregationDiscriminatorImpl;
...
public class LoginTotalsStatisticsController
    extends BaseSimpleGroupedStatisticsReportController<
        LoginAggregation, LoginAggregationKey, LoginAggregationDiscriminator, LoginReportForm> {

  private static final String DATA_TABLE_RESOURCE_ID = "loginData";
  private static final String REPORT_NAME = "login.totals";

  @Autowired private LoginAggregationDao<LoginAggregation> loginDao;
  ...
  public ModelAndView renderLoginAggregationReport(LoginReportForm form)
    ...
    if (form.isTotalLogins() && form.isUniqueLogins()) {
      return ImmutableList.of(
          ...
          new ColumnDescription(
              groupName + "-uniqueLogins", ValueType.NUMBER, groupName + " - Unique Logins"),
          new ColumnDescription(
              groupName + "-totalLogins", ValueType.NUMBER, groupName + " - Total Logins"));
  ...
  @Override
  protected LoginAggregationDiscriminator createGroupedDiscriminatorInstance(
