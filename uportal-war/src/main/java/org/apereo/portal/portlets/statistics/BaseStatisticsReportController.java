...
public abstract class BaseStatisticsReportController<
  ...
  /**
   * List of intervals in the preferred report order. This is the order they are tested against the
   * results of {@link #getIntervals()}. The first hit is used to populate the default form.
   */
  ...
  /**
   * Set the start/end date and the interval to have selected by default if they are not already set
   */
   ...
   * of aggregation data points. Subclasses may override this method to obtain more from the form
   * than just AggregatedGroupMappings as report columns.
   *
  ...
  /**
   * Return additional data to attach to the title of the form. This is used when the user selects a
   * single value of a multi-valued set and you don't want to include the selected value in the
  ...
  /**
   * Returns true to indicate report format is only data table and doesn't have report graph titles,
   * etc. so the report columns needs to fully describe the data columns. CSV and HTML tables
   * require full column header descriptions.
   *
   * @param form the form
        ...
        case DATE:
          {
            dateTimeValue =
                new DateValue(
                    rowTime.getYear(), rowTime.getMonthOfYear() - 1, rowTime.getDayOfMonth());
            break;
          }
        case TIMEOFDAY:
          {
            dateTimeValue =
                new TimeOfDayValue(rowTime.getHourOfDay(), rowTime.getMinuteOfHour(), 0);
            break;
          }
        default:
          {
