...
public abstract class QueueingEventHandler<E extends ApplicationEvent>
  ...
  /**
   * Flushes the queued PortalEvents to the configured {@link BatchingEventHandler}. If <code>force
   * </code> is false flushing only happens if there are enough events in the queue and a flush
   * isn't already under way. If <code>force</code> is true all queued events will be flushed and
   * the calling thread will wait until any previously executing flush call completes before
   * flushing
   *
   * @param force Forces flushing events to the {@link BatchingEventHandler} even if there are fewer
   *     than <code>flushCount</code> PortalEvents in the queue.
   */
    ...
    try {
      while (!this.eventQueue.isEmpty()) {
        ...
        try {
          ...
          this.logger.error(
              "An exception was thrown while trying to flush " + eventBuffer.size() + " events", t);
