...
public final class UserFilteringPortalEventHandler<E extends PortalEvent>
   ...
   * supported otherwise exact String equality matching is done to determine ignored userNames. The
   * property defaults to null (all user names)
   *
  ...
  /**
   * If no <code>ignoredUserNamePatterns</code> {@link Collection} is configured all user-names are
   * supported otherwise exact regex pattern matching is done to determine ignored userNames. The
   * property defaults to null (all user names)
   *
   ...
   * property is false only {@link ApplicationEvent}s where {@link IPerson#isGuest()} is false will
   * be supported. The property defaults to true.
   */
