...
public final class EventDateTimeUtils {
   ...
   *     Returns -1 if the range comes before the instant: (getEnd() <= instant) Returns 1 if the
   *     range comes after the instant (instant < getStart())
   ...
   *   <li>Once placed in order the start of the current quarter is equal to the end of the previous
   *       quarter
  ...
  /**
   * Same function as {@link #findDateRange(ReadableInstant, Collection)} optimized for working on a
   * pre-sorted List of date ranges by doing a binary search. The List must be sorted by {@link
   * DateRange#getStart()}
   */
