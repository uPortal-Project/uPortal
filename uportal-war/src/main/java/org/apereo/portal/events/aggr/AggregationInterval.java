...
public enum AggregationInterval {
  ...
  /**
   * @return the {@link DateTimeFieldType} for the {@link AggregationInterval}, null if there is no
   *     mapping
   */
  ...
  public int determineIntervalsBetween(ReadableInstant start, ReadableInstant end) {
    ...
    return (int)
        Math.round(
            durationType.getField(chrono).getDifference(end.getMillis(), start.getMillis())
                / ratio);
  }
  ...
  public DateTime determineEnd(DateTime instant) {
    ...
    if (this == AggregationInterval.FIVE_MINUTE) {
      final DateTime start =
          instant.hourOfDay().roundFloorCopy().plusMinutes((instant.getMinuteOfHour() / 5) * 5);
