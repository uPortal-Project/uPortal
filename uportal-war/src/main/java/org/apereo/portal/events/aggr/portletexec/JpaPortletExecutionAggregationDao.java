...
public class JpaPortletExecutionAggregationDao
  ...
  protected void addAggregationSpecificKeyPredicate(
      ...
      List<Predicate> keyPredicates) {
    keyPredicates.add(
        root.get(PortletExecutionAggregationImpl_.aggregatedPortlet).in(portletMappingParameter));
    keyPredicates.add(
        root.get(PortletExecutionAggregationImpl_.executionType).in(executionTypeParameter));
  }
  ...
  protected void bindAggregationSpecificKeyParameters(
      TypedQuery<PortletExecutionAggregationImpl> query, Set<PortletExecutionAggregationKey> keys) {
  ...
  @Override
  protected void bindAggregationSpecificKeyParameters(
      NaturalIdQuery<PortletExecutionAggregationImpl> query, PortletExecutionAggregationKey key) {
    query.using(
  ...
  protected PortletExecutionAggregationImpl createAggregationInstance(
    ...
    return new PortletExecutionAggregationImpl(
        timeDimension, dateDimension, interval, aggregatedGroup, portletMapping, executionType);
  }
