...
/**
 * Handles creation and maintenance of the date and time dimensions used when aggregating portal
 * events
 */
public interface PortalEventDimensionPopulator {
   ...
   */
  static final String DIMENSION_LOCK_NAME =
      PortalEventDimensionPopulator.class.getName() + ".DIMENSION_LOCK";

  /**
   * @return true if {@link #doPopulateDimensions()} has been called and has completed successfully
   ...
   * aggregation tools. <br>
   * Note that this method MUST be called while the current thread & JVM owns the {@link
   * #DIMENSION_LOCK_NAME} cluster wide lock via the {@link IClusterLockService}
   */
