...
public class DefaultTinCanAPIProvider implements ITinCanAPIProvider {
  ...
  public void init() {
    ...
    try {
      ...
      ResponseEntity<Object> response =
          sendRequest(STATES_REST_ENDPOINT, HttpMethod.POST, getParams, body, Object.class);
  ...
  public boolean sendEvent(LrsStatement statement) {
    ...
    ResponseEntity<Object> response =
        sendRequest(STATEMENTS_REST_ENDPOINT, HttpMethod.POST, null, statement, Object.class);
    if (response.getStatusCode().series() == Series.SUCCESSFUL) {
   ...
   * configuration out to portal.properties. It reads dynamically named properties based on the "id"
   * of this LRS provider. This is similar to the way that the TinCan configuration is handled for
   * Sakai.
   */
  protected void loadConfig() {
    ...
    actorEmail =
        propertyResolver.getProperty(format(PROPERTY_FORMAT, id, "actor-email"), actorEmail);
    activityId =
        propertyResolver.getProperty(format(PROPERTY_FORMAT, id, "activity-id"), activityId);
    ...
    activitiesFormParamName =
        propertyResolver.getProperty(
            format(PROPERTY_FORMAT, id, "activity-form-param-name"), activitiesFormParamName);
