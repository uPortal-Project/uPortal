...
public interface IPortalEventDao {
  ...
  /**
   * Gets all un-aggregated persisted events in the time range. After the handler is called on each
   * event it is marked as aggregated. To deal with memory and data access issues the results are
   * not returned but passed in order to the provided {@link Function} handler. If aggregation
   * should stop the handler should return false after processing an event. Only events processed up
   * to that point will be marked as aggregated.
   *
   * @param startTime The inclusive start time to get events for
   * @param endTime The exclusive end time to get events for
   * @param maxEvents The maximum number events to retrieve. -1 means no limit
