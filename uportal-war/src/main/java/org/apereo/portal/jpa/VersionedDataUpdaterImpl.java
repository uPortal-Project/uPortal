...
public class VersionedDataUpdaterImpl implements VersionedDataUpdater {
    ...
    @Autowired
    public void setVersionedDatabaseUpdateHelpers(
            Collection<IVersionedDatabaseUpdateHelper> helpers) {
        this.databaseUpdateHelpers =
                new HashMap<String, SortedSet<IVersionedDatabaseUpdateHelper>>();

        for (final IVersionedDatabaseUpdateHelper helper : helpers) {
            final String databaseName = helper.getDatabaseName();

            SortedSet<IVersionedDatabaseUpdateHelper> updaters =
                    this.databaseUpdateHelpers.get(databaseName);
            if (updaters == null) {
                updaters =
                        new TreeSet<IVersionedDatabaseUpdateHelper>(
                                VersionedDatabaseUpdateHelperComparator.INSTANCE);
    ...
    public void preUpdateDatabase(String product) {
        ...
        final SortedSet<IVersionedDatabaseUpdateHelper> updateHelpers =
                this.databaseUpdateHelpers.get(product);
    ...
    protected Version getAndVerifyDatabaseVersionForUpdate(String product) {
        ...
        if (!VersionUtils.canUpdate(dbVersion, codeVersion)) {
            throw new IllegalStateException(
                    "Cannot update " + product + " db from " + dbVersion + " to " + codeVersion);
        }
    ...
    private static final class VersionedDatabaseUpdateHelperComparator
            implements Comparator<IVersionedDatabaseUpdateHelper> {
        public static final VersionedDatabaseUpdateHelperComparator INSTANCE =
                new VersionedDatabaseUpdateHelperComparator();
