  ...
  private TenantOperationResponse prepareResponse(final ITenant tenant) {
    try {
      ...
      final TenantOperationResponse error =
          new TenantOperationResponse(this, TenantOperationResponse.Result.FAIL); // Just a warning
      error.addMessage(
    ...
    rslt.addMessage(
        createLocalizedMessage(
            TENANT_ADMIN_EMAIL_SENT, new String[] {tenant.getAttribute(ADMIN_CONTACT_EMAIL)}));
    return rslt;
  }
