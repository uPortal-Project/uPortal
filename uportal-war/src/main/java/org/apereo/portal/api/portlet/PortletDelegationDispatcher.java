...
public interface PortletDelegationDispatcher {
  /**
   * @return The ID of the delegate portlet window, useful for retrieving this dispatcher again from
   *     the {@link PortletDelegationLocator}
   */
  ...
  /**
   * Executes a portlet action request on the delegate window. The state, mode and parameters in the
   * delegation request (if set) are used by the delegate.
   *
  ...
  /**
   * Executes a portlet render request on the delegate window. The state, mode and parameters in the
   * delegation request (if set) are used by the delegate. The output of the portlet's rendering is
   * written to the provided {@link PortletOutputHandler}
   *
