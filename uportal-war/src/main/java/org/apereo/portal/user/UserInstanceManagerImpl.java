...
public class UserInstanceManagerImpl implements IUserInstanceManager {
  ...
  public IUserInstance getUserInstance(HttpServletRequest request) throws PortalException {
    ...
    if (session == null) {
      throw new IllegalStateException(
          "HttpServletRequest.getSession() returned a null session for request: " + request);
    }
  ...
  protected IUserProfile getUserProfile(
      HttpServletRequest request, IPerson person, LocaleManager localeManager, String userAgent) {
   ...
   * The manager can deal with this class returning a null value and its field is transient so the
   * session can be serialized successfully with the UserInstance object in it.
   *
