...
public final class PortletUtils {
  ...
  /**
   * A static EL function that is defined in portletUrl.tld Takes a portletUrl object and coverts it
   * into an actual Url Example is in search, returns a marketplace entry Url
   *
   * @param portletUrl
   * @param request
  ...
  public static String getStringFromPortletUrl(PortletUrl portletUrl, HttpServletRequest request) {
    ...
    IPortletWindow marketplaceWindow =
        portletWindowRegistry.getOrCreateDefaultPortletWindowByFname(
            request, MarketplacePortletDefinition.MARKETPLACE_FNAME);
