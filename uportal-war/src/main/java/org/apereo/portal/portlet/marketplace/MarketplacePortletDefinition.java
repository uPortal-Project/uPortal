...
public class MarketplacePortletDefinition implements IPortletDefinition {
    ...
    for (int i = 0; i < urls.size(); i++) {
      if (urls.get(i) != null) {
        if (captions.size() > i && captions.get(i) != null) {
          screenShots.add(
              new ScreenShot(
                  urls.get(i).getValues()[0], Arrays.asList(captions.get(i).getValues())));
        } else {
      ...
      for (IPortletDefinition portletDefinition : portletsInCategory) {
        allRelatedPortlets.add(
            new MarketplacePortletDefinition(
                portletDefinition, this.marketplaceService, this.portletCategoryRegistry));
      }
    }
  ...
  /**
   * @return a set of categories that includes all the categories that this definition is in and all
   *     parent categories of the portlet's category. Will not return null. Might return empty set.
   */
  ...
  /**
   * Obtain up to QUANTITY_RELATED_PORTLETS_TO_SHOW random related portlets BROWSEable by the given
   * user.
   *
   ...
   * this method will *ONLY* work when invoked in the context of a Spring-managed HttpServletRequest
   * available via RequestContextHolder.
   *
   * <p>When there is an alternativeMaximizedLink, this method returns that instead.
   *
   * <p>WARNING: This method does not consider whether the requesting user has permission to render
