...
public class MarketplaceService implements IMarketplaceService, ApplicationListener<LoginEvent> {
   ...
   * @return a Future that will resolve to a set of MarketplaceEntry objects the requested user has
   *     browse access to.
  ...
  public Future<ImmutableSet<MarketplaceEntry>> loadMarketplaceEntriesFor(
    ...
    logger.trace(
        "These portlet definitions {} are browseable by {}.", visiblePortletDefinitions, user);
  ...
  public Set<PortletCategory> browseableNonEmptyPortletCategoriesFor(
    ...
    final Set<MarketplaceEntry> browseablePortlets =
        browseableMarketplaceEntriesFor(user, categories);
    ...
    for (final MarketplaceEntry entry : browseablePortlets) {
      ...
      for (final PortletCategory category :
          this.portletCategoryRegistry.getParentCategories(portletDefinition)) {
        ...
        if (mayBrowse(principal, categoryId)) {
          ...
          logger.trace(
              "Portlet {} is browseable by {} but it is in category {} "
  ...
  public boolean mayBrowsePortlet(
      final IAuthorizationPrincipal principal, final IPortletDefinition portletDefinition) {
    Validate.notNull(principal, "Cannot determine if null principals can browse portlets.");
    Validate.notNull(
        portletDefinition, "Cannot determine whether a user can browse a null portlet definition.");
  ...
  /**
   * True if the principal has UP_PORTLET_SUBSCRIBE.BROWSE on the target id. The target ID must be
   * fully resolved. This method will not e.g. prepend the portlet prefix to target ids that seem
   * like they might be portlet IDs.
   *
   * <p>Implementation note: technically this method is not stateless since asking an
   * AuthorizationPrincipal about its permissions has caching side effects in the permissions
   * system, but it's stateless as far as this Service is concerned.
   *
  ...
  private static boolean mayBrowse(final IAuthorizationPrincipal principal, final String targetId) {
    ...
    return (principal.hasPermission(
        IPermission.PORTAL_SUBSCRIBE, IPermission.PORTLET_BROWSE_ACTIVITY, targetId));
  }

  /** Called recursively to gather all specified categories and descendants */
  private void collectSpecifiedAndDescendantCategories(
