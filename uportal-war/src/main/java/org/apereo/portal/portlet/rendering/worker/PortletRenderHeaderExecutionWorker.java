...
class PortletRenderHeaderExecutionWorker extends PortletExecutionWorker<PortletRenderResult>
  ...
  /* (non-Javadoc)
   * @see org.apereo.portal.portlet.rendering.worker.IPortletRenderExecutionWorker#getOutput(long)
   */
  @Override
  ...
  /**
   * Obtain the RENDER_HEADER output. Note that uPortal supports the model of obtaining HTML markup
   * from the portlet, whether from a Spring view or servlet output and inserting it within the HTML
   * HEAD section of the page. uPortal does not support the model where the portlet creates DOM
   * elements to be written to the HEAD section and constructs the HTML output; e.g. the following
   * does NOT work in uPortal:
   *
   * <p>
   *
   * <pre>
  ...
  protected PortletRenderResult callInternal() throws Exception {
    ...
    final PortletRenderResult result =
        portletRenderer.doRenderHeader(
            portletWindowId, request, response, renderPortletOutputHandler);
