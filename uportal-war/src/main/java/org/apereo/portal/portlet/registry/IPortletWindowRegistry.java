...
public interface IPortletWindowRegistry {
   ...
   * exists for the parameters a new window will be created and returned. This is a convenience for
   * {@link #getDefaultPortletWindowId(IPortletEntityId)}, {@link
  ...
  /**
   * Get the portlet window object for the rendering pipeline start element and return a replacement
   * start element that contains the portlet window id
   *
   * @return The found portlet window and a StartElement to replace the parameter which will contain
   *     the resolved portlet window id
   */
