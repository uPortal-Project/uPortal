...
public interface IPortletCacheControlService {
  ...
  CacheState<CachedPortletResourceData<Long>, Long> getPortletResourceState(
  ...
  CacheState<CachedPortletData<PortletRenderResult>, PortletRenderResult> getPortletRenderState(
      HttpServletRequest request, IPortletWindowId portletWindowId);

  /**
   * This method checks the {@link CacheControl} to determine if the output should be captured for
   * caching.
   *
  ...
  /**
   * Store the output of a render header request in the cache for the portlet and request. This
   * method internally will determine if the content needs to be stored in a public or private
   * scoped cache.
   */
  void cachePortletRenderHeaderOutput(
  ...
  /**
   * Store the output of a render request in the cache for the portlet and request. This method
   * internally will determine if the content needs to be stored in a public or private scoped
   * cache.
   */
  void cachePortletRenderOutput(
      IPortletWindowId portletWindowId,
      HttpServletRequest httpRequest,
      CacheState<CachedPortletData<PortletRenderResult>, PortletRenderResult> cacheState,
      CachedPortletData<PortletRenderResult> cachedPortletData);
  ...
  /**
   * Store the output of a resource request in the cache for the portlet and request. This method
   * internally will determine if the content needs to be stored in a public or private scoped
   * cache.
   */
  void cachePortletResourceOutput(
      IPortletWindowId portletWindowId,
      HttpServletRequest httpRequest,
   ...
   *
   * <p>Should be called on any Action or Event Request.
   */
  boolean purgeCachedPortletData(IPortletWindowId portletWindowId, HttpServletRequest httpRequest);
}
