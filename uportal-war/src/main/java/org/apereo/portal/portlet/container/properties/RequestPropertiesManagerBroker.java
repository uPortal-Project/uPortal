...
/**
 * Delegates properties related calls to a {@link List} of {@link IRequestPropertiesManager}s in
 * order. No exception handling is done so any exception in a child IRequestPropertiesManager will
 * cause the call to fail. <br>
 * <br>
 * When {@link #getRequestProperties(HttpServletRequest, IPortletWindow)} is called the returned
...
public class RequestPropertiesManagerBroker implements IRequestPropertiesManager {
    ...
    @Autowired
    public void setPropertiesManagers(
            Collection<? extends IRequestPropertiesManager> propertiesManagers) {
    ...
    public boolean setResponseProperty(
        ...
        for (final IRequestPropertiesManager propertiesManager : this.propertiesManagers) {
            if (propertiesManager.setResponseProperty(
                    portletRequest, portletWindow, property, value)) {
                return true;
            }
        }
    ...
    public boolean addResponseProperty(
        ...
        for (final IRequestPropertiesManager propertiesManager : this.propertiesManagers) {
            if (propertiesManager.addResponseProperty(
                    portletRequest, portletWindow, property, value)) {
                return true;
            }
        }
    ...
    public <P extends Populator<String, String>> void populateRequestProperties(
        ...
        for (final IRequestPropertiesManager propertiesManager : this.propertiesManagers) {
            propertiesManager.populateRequestProperties(
                    portletRequest, portletWindow, propertiesPopulator);
        }
    }
}
