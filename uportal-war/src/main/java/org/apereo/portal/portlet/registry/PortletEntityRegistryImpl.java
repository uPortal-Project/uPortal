...
public class PortletEntityRegistryImpl implements IPortletEntityRegistry {
  ...
  public IPortletEntity getOrCreatePortletEntity(
    ...
    if (channelNode == null) {
      this.logger.warn(
          "No layout node exists for id " + layoutNodeId + ", no portlet entity will be returned.");
      return null;
    }
    ...
    if (portletDefinition != null) {
      ...
      return this.getOrCreatePortletEntity(
          request, portletDefinition.getPortletDefinitionId(), layoutNodeId, person.getID());
    }
  ...
  public IPortletEntity getOrCreatePortletEntityByFname(
    ...
    try {
      final IPortletEntity portletEntity =
          this.getOrCreatePortletEntity(request, userInstance, preferredChannelSubscribeId);
  ...
  public IPortletEntity getOrCreateDelegatePortletEntity(
    ...
    final String layoutNodeId =
        PortletWindowIdStringUtils.convertToDelegateLayoutNodeId(parentPortletWindowId.toString());
        ...
        if (shouldBePersisted) {
          try {
            ...
            final boolean exists =
                this.portletEntityDao.portletEntityExists(persistentEntity.getPortletEntityId());
            if (!exists) {
              this.logger.warn(
  ...
  protected IPortletDefinition getPortletDefinition(
    ...
    final ConcurrentMap<String, IPortletDefinition> portletDefinitions =
        PortalWebUtils.getMapRequestAttribute(request, PORTLET_DEFINITION_LOOKUP_MAP_ATTRIBUTE);
  ...
  protected IPortletEntity createPersistentEntity(
    ...
    if (persistentEntity != null) {
      ...
      persistentEntity =
          this.portletEntityDao.createPortletEntity(portletDefinitionId, layoutNodeId, userId);
    }
