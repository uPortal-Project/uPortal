...
public abstract class ThreadLocalPortletExecutionInterceptor<T>
  ...
  private static final String CURRENT_THREAD =
      ThreadLocalPortletExecutionInterceptor.class.getName() + ".CURRENT_THREAD";

  private String contextAttributeName = this.getClass().getName();

  /** Get the current value from the ThreadLocal store */
  protected abstract T getThreadLocalValue(
      HttpServletRequest request, HttpServletResponse response, IPortletExecutionContext context);

  /**
   * Set the specified value into the ThreadLocal store. If the value is null the store may remove
   * the ThreadLocal reference
   */
  protected abstract void setThreadLocalValue(
  ...
  @Override
  public final void preExecution(
      HttpServletRequest request, HttpServletResponse response, IPortletExecutionContext context) {
  ...
  @Override
  public final void postExecution(
      HttpServletRequest request,
      HttpServletResponse response,
      IPortletExecutionContext context,
      Exception e) {
