...
public class PortletCookieServiceImpl implements IPortletCookieService, ServletContextAware {
  ...
  /**
   * Name of the {@link HttpSession} attribute used for storing a concurrent map of portlet cookies
  ...
  /**
   * Name of the {@link HttpSession} attribute used to track the value of the {@link IPortalCookie}
   * (useful if the client does not accept cookies). Note: Package access for the test class
   * convenience.
   */
  ...
  /**
   * @param maxAgeUpdateInterval How frequently (in ms) the maxAge date on the portal cookie should
   *     be updated. Defaults to 5 minutes. Only portal cookies older than 5 minutes will be updated
   *     in the client's browser and the db with a new maxAge
   */
  ...
  public void updatePortalCookie(HttpServletRequest request, HttpServletResponse response) {
    ...
    final Cookie cookie =
        this.convertToCookie(portalCookie, this.portalCookieAlwaysSecure || request.isSecure());
  ...
  public Cookie[] getAllPortletCookies(
    ...
    if (servletCookies == null) {
      ...
      for (int i = 0; i < servletCookies.length; i++) {
        if (servletCookies[i].getName().equals(this.cookieName)) {
          ...
          servletCookies[i] =
              convertToCookie(portalCookie, this.portalCookieAlwaysSecure || request.isSecure());
        }
      }
    }
    ...
    final Cookie[] cookies =
        new Cookie[servletCookies.length + portletCookies.size() + sessionOnlyCookies.size()];
  ...
  /**
   * Get the {@link Map} of {@link SessionOnlyPortletCookieImpl}s stored in the {@link HttpSession}
   * specifically used for storing {@link SessionOnlyPortletCookieImpl}s with a maxAge equal to -1.
   ...
   * HttpServletRequest}.
   *
   * <p>Returns the {@link Cookie} if a match is found in the request, otherwise gracefully returns
   * null.
   *
   * @param name
   * @param request
