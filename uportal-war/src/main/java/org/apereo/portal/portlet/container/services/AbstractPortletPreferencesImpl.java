...
public abstract class AbstractPortletPreferencesImpl<C> implements PortletPreferences {
  ...
  protected abstract void loadTargetPortletPreferences(
      C initContext, Map<String, IPortletPreference> targetPortletPreferences);

  protected abstract void loadBasePortletPreferences(
      C initContext, Map<String, IPortletPreference> basePortletPreferences);
  ...
  private final void initPrefs() {
    ...
    this.compositePreferences =
        new LinkedHashMap<String, IPortletPreference>(this.basePortletPreferences);
    this.compositePreferences.putAll(this.targetPortletPreferences);

    this.compositePreferencesView =
        Maps.transformValues(
            this.compositePreferences,
            new Function<IPortletPreference, String[]>() {
              @Override
  ...
  public final Enumeration<String> getNames() {
    final Map<String, String[]> compositePortletPreferencesView =
        this.getCompositePortletPreferencesView();
  ...
  public final void store() throws IOException, ValidatorException {
    ...
    if (!modified) {
      logger.debug(
          "Skipping store of portlet preferences, nothing has changed: {}", getLogDescription());
      return;
    }
    ...
    if (stored) {
      logger.debug(
          "Store of portlet preferences resulted in a change, clear preferences cache: {}",
          getLogDescription());
      this.clearPrefs();
    }
