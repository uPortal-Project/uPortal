...
public class PortletRendererImpl implements IPortletRenderer {
  ...
  public long doEvent(
    ...
    try {
      this.portletContainer.doEvent(
          portletWindow.getPlutoPortletWindow(), httpServletRequest, httpServletResponse, event);
    } catch (PortletException pe) {
      throw new PortletDispatchException(
          "The portlet window '" + portletWindow + "' threw an exception while executing event.",
          portletWindow,
          pe);
    } catch (PortletContainerException pce) {
      throw new PortletDispatchException(
  ...
  protected PortletRenderResult doRender(
    ...
    if (cacheState.isUseCachedData()) {
      return doRenderReplayCachedContent(
          portletWindow, httpServletRequest, cacheState, portletOutputHandler, renderPart, 0);
    }
    ...
    if (cacheState != null) {
      ...
      if (shouldCache) {
        final CachedPortletData<PortletRenderResult> cachedPortletData =
            cachingPortletOutputHandler.getCachedPortletData(portletRenderResult, cacheControl);
  ...
  /**
   * Construct a {@link PortletRenderResult} from information in the {@link HttpServletRequest}. The
   * second argument is how long the render action took.
   *
  ...
  protected PortletRenderResult constructPortletRenderResult(
    ...
    final String newItemCountString =
        (String)
            httpServletRequest.getAttribute(IPortletRenderer.ATTRIBUTE__PORTLET_NEW_ITEM_COUNT);
  ...
  public long doServeResource(
    ...
    if (cacheState != null) {
      ...
      if (shouldCache) {
        final CachedPortletResourceData<Long> cachedPortletResourceData =
            cachingPortletOutputHandler.getCachedPortletResourceData(executionTime, cacheControl);
  ...
  public void doReset(
    ...
    if (portletWindow != null) {
      ...
      httpServletRequest.setAttribute(
          PortletSessionAdministrativeRequestListener.SCOPE, PortletSession.PORTLET_SCOPE);
      ...
      httpServletRequest.setAttribute(
          ATTRIBUTE__PORTLET_OUTPUT_HANDLER, new RenderPortletOutputHandler(characterEncoding));
  ...
  protected HttpServletRequest setupPortletRequest(HttpServletRequest httpServletRequest) {
    ...
    portletHttpServletRequestWrapper.setAttribute(
        PortletDelegationLocator.PORTLET_DELECATION_LOCATOR_ATTR, this.portletDelegationLocator);
   ...
   * the PortletWindow specifies config mode, throws AuthorizationException. Otherwise does nothing.
   *
