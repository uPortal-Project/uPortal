...
public final class PortletCachingHeaderUtils {
  ...
  public static void setCachingHeaders(
    ...
    final int maxAge =
        (int) TimeUnit.MILLISECONDS.toSeconds(expirationTime - System.currentTimeMillis());
  ...
  /**
   * Set the Last-Modified, CacheControl, and Expires headers based on the maxAge, publicScope and
   * lastModified.
   *
   * @param maxAge Maximum age for the content, follows the portlet rules (-1 cache forever, 0 cache
   *     never, N cache seconds)
  ...
  public static void setETag(
      String token, PortletResourceOutputHandler portletResourceOutputHandler) {
