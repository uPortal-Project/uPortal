...
public interface IPortletExecutionWorker<V> extends IPortletExecutionContext {
  ...
  /**
   * Submit the worker for execution. The worker may start immediately or submit itself to a thread
   * pool or service for execution at a future time.
   *
   ...
   *
   * @param timeout The maximum time in ms to wait for the worker to complete, if the worker has not
   *     started yet this includes time spent waiting to start
