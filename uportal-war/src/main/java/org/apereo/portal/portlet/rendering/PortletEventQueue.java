...
public class PortletEventQueue implements Iterable<IPortletWindowId> {
  ...
  /**
   * Map for portlet windows where the value is a queue of events that need to be dispatched to the
   * portlet
   */
  private final ConcurrentMap<IPortletWindowId, Queue<QueuedEvent>> resolvedEventQueues =
    ...
    if (events == null) {
      events =
          ConcurrentMapUtils.putIfAbsent(
              resolvedEventQueues, portletWindowId, new ConcurrentLinkedQueue<QueuedEvent>());
    }
