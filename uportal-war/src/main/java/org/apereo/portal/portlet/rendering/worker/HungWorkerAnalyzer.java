...
public class HungWorkerAnalyzer
    ...
    if (numberPermittedErrantByFname != null) {
      if (numberPermittedErrantByFname == 0) {
        ...
        this.percentPermittedErrantByFname =
            ((double) numberPermittedErrantByFname) / this.portletThreadPool.getMaximumPoolSize();
      }
    }
  }
  ...
  protected void countHungWorker(IPortletExecutionWorker<?> worker) {
    ...
    if (count == null) {
      count =
          ConcurrentMapUtils.putIfAbsent(this.hungPortletCounts, portletFname, new AtomicInteger());
    }
  ...
  private void logState(final String portletFname, final int hungWorkerCount) {
    ...
    final Object[] args =
        new Object[] {
          portletFname, hungWorkerCount, maximumPoolSize, availableWorkers, hungWorkerLimit
        };
