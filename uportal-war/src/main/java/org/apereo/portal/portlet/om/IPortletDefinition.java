...
public interface IPortletDefinition extends IBasicEntity, IPortalData {
   ...
   *
   * <p>This is useful for portlets that when maximized ought to instead be the external URL or web
   * application that they're representing in the portal.
   */
   ...
   * if none.
   *
   * <p>Syntactic sugar for parsing potential alternative maximized link as a preferable alternative
  ...
  /**
   * @param actionTimeout Optional timeout for action requests in ms, if null {@link #getTimeout()}
  ...
  /**
   * @param renderTimeout Optional timeout for render requests in ms, if null {@link #getTimeout()}
   *     will be used, -1 means no timeout.
   */
  ...
  /**
   * Equals must be able to compare against any other {@link IPortletDefinition} and the comparison
   * must only use the fname
   */
  @Override
