...
public class LocaleManager implements Serializable {
  ...
  private Locale[] loadPortalLocales() {
    String portalLocalesString =
        PropertiesManager.getProperty("org.apereo.portal.i18n.LocaleManager.portal_locales");
   ...
   * The following priority is given: session, user, browser, portal, and jvm.
   *
  ...
  private void addToLocaleList(List localeList, Locale[] locales) {
    if (locales != null) {
      for (int i = 0; i < locales.length; i++) {
        if (locales[i] != null && !localeList.contains(locales[i])) localeList.add(locales[i]);
      }
    }
  }
  ...
  /**
   * Constructs a comma-delimited list of locales that could be parsed back into a Locale array with
   ...
   * element representing the selected locale will have an attribute of selected with value of true.
   * This is helpful when constructing user interfaces that indicate which locale is selected.
   *
   * @param locales the locale list
