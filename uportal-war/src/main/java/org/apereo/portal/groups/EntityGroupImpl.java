...
public class EntityGroupImpl extends GroupMemberImpl implements IEntityGroup {
  ...
  public EntityGroupImpl(String groupKey, Class<? extends IBasicEntity> entityType)
    ...
    this.childrenCache = cacheManager.getCache("org.apereo.portal.groups.EntityGroupImpl.children");
  }
  /**
  ...
  /**
   * A member must share the <code>entityType</code> of its containing <code>IEntityGroup</code>. If
   * it is a group, it must have a unique name within each of its containing groups and the
   * resulting group must not contain a circular reference. Removed the requirement for unique group
   * names. (03-04-2004, de)
   *
   * @param gm org.apereo.portal.groups.IGroupMember
   ...
   * cache, adding to the copy, and then replacing the original with the copy. At this point, <code>
   * gm</code> does not yet have <code>this</code> in its containing group cache. That cache entry
  ...
  /**
   * Removes the <code>IGroupMember</code> key from the appropriate key cache, by copying the cache,
   * removing the key from the copy and replacing the original with the copy. At this point, <code>
   * gm</code> still has <code>this</code> in its containing groups cache. That cache entry is not
   * removed until update(), when changes are committed to the store.
   *
   * @param gm org.apereo.portal.groups.IGroupMember
   */
