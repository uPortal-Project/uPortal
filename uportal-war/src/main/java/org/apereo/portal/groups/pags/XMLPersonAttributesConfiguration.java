...
/**
 * Read in the configuration for the Person Attributes group store using an XML file. See the
 * example file in /properties/groups/PAGSGroupStoreConfig.xml and the DTD in
 * /webpages/dtd/PAGSGroupStore.dtd for information on the file format.
 *
 * @author Al Wold
        ...
        try {
            config =
                    ResourceLoader.getResourceAsDocument(
                            this.getClass(), "/properties/groups/PAGSGroupStoreConfig.xml", true);
        } catch (Exception rme) {
            throw new RuntimeException(
                    "PersonAttributesGroupStore: Unable to read configuration document");
        }
            ...
            if (children.item(i) instanceof Element) {
                ...
                if (tagName.equals("group-key")) {
                    ...
                    for (int j = 0; j < testGroups.getLength(); j++) {
                        ...
                        if (testGroup instanceof Element
                                && ((Element) testGroup).getTagName().equals("test-group")) {
                            ...
                            for (int k = 0; k < tests.getLength(); k++) {
                                ...
                                if (test instanceof Element
                                        && ((Element) test).getTagName().equals("test")) {
                                    ...
                                    IPersonTester testerInst =
                                            initializeTester(tester, attribute, value);
