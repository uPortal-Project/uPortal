...
public final class SmartLdapGroupStore implements IEntityGroupStore {
   ...
   *   <li>Map of all 'keys' (DNs) of SmartLdap managed groups indexed by group name in upper case.
   *       Includes ROOT_GROUP.
   * </ul>
   */
  ...
  /**
   * Returns an <code>Iterator</code> over the <code>Collection</code> of <code>IEntityGroups</code>
   * that the <code>IGroupMember</code> belongs to.
   *
  ...
  public boolean hasUndiscoveredChildrenWithinDn(
    ...
    log.trace(
        "Query for children of parent group '{}':  {}", record.getGroup().getLocalKey(), rslt);
  ...
  /**
   * Verifies that the collection of groups needs rebuilding and, if so, spawns a new worker <code>
   * Thread</code> for that purpose.
   */
