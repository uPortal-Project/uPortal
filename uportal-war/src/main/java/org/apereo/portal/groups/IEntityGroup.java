...
public interface IEntityGroup extends IGroupMember {
  ...
  /**
   * Adds <code>IGroupMember</code> gm to this group, but does not commit it to the data store. Use
   * <code>updateMembers()</code> to commit memberships to the data store.
   *
   * @param gm org.apereo.portal.groups.IGroupMember
  ...
  /**
   * Removes the <code>IGroupMember</code> from this group, but does not remove the membership from
   * the data store.
   *
   * @param gm org.apereo.portal.groups.IGroupMember
   */
