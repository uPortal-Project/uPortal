...
public class ReferenceIndividualGroupService extends ReferenceCompositeGroupService
   ...
   * </code> block tries to release any groups that are still locked, which can occur if an attempt
   * to remove the group from one of its containing groups fails and throws a GroupsException. In
   * this event, we do not try to roll back any successful removes, since that would probably fail
   * anyway.
   *
  ...
  /**
   * Returns an <code>IEntity</code> representing a portal entity. This does not guarantee that the
   * underlying entity actually exists.
   */
  ...
  /**
   * Returns the implementation of <code>IEntityGroupStore</code> whose class name was retrieved by
   * the PropertiesManager (see initialize()).
   */
  ...
  private void initialize() throws GroupsException {
    ...
    if (groupStoreFactoryName == null) {
      ...
      try {
        IEntityGroupStoreFactory groupStoreFactory =
            (IEntityGroupStoreFactory) Class.forName(groupStoreFactoryName).newInstance();
    ...
    if (entityStoreFactoryName == null) {
      ...
      try {
        ...
        eMsg =
            "ReferenceIndividualGroupService.initialize(): Failed to instantiate entity store " + e;
        log.error(eMsg);
        throw new GroupsException(eMsg, e);
      }
    }
    ...
    if (entitySearcherFactoryName == null) {
      ...
      try {
        IEntitySearcherFactory entitySearcherFactory =
            (IEntitySearcherFactory) Class.forName(entitySearcherFactoryName).newInstance();
   ...
   * Then update the cache to invalidate copies on peer servers.
   *
   * @param group ILockableEntityGroup
   */
