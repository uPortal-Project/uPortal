...
public class ChunkingEventReader extends BaseXMLEventReader {
  ...
  protected void chunkString(
      final List<CharacterEvent> characterEvents, final CharSequence buffer, int patternIndex) {
    ...
    for (; patternIndex < this.chunkingPatterns.length; patternIndex++) {
      ...
      if (matcher.find()) {
        ...
        do {
          ...
          this.chunkString(
              characterEvents, buffer.subSequence(prevMatchEnd, matcher.start()), patternIndex + 1);
        ...
        if (prevMatchEnd < buffer.length()) {
          this.chunkString(
              characterEvents, buffer.subSequence(prevMatchEnd, buffer.length()), patternIndex + 1);
        }
