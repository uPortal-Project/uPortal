 ...
 * 
 * A portlet entity ID string consists of three parts separated by underscores.
 * The format is:  <portlet-entity-id>_<layout-node-id>_<user-id>
 * 
 * Examples:
 *   88_n149_52
 *   71_u54_12
 *  
 * Note that if a portlet has a 'delegate' portlet, then the 'layout-node-id' will have the 
 * format:  dlg-<delegate-portlet-window-id-with-underscores-replaced-by-dashes>
 * 
 * Examples:
 *   90_dlg-5-ctf1-18_18      // here the portlet window ID of the delegate portlet is:  5_ctf1_18
 *   75_dlg-10-ctf2-32.tw_32  // here the portlet entity ID of the delegate portlet is:  10_ctf2_32.tw
 * 
 * Currently, the admin interface uses Spring Webflow with 'delegate' portlets to display portlet configuration
 * views.  When the admin interface portlet content is fetched with an 'exclusive' window state URL, the layout 
 * node ID includes the "tw" transient (stateless) window instance ID.  This behavior was observed with RespondrJS 
...
public class PortletEntityIdStringUtils {
    ...
    public static String format(final String portletDefinitionId, final String layoutNodeId, final String userId) {
        return portletDefinitionId + ID_PART_SEPARATOR + layoutNodeId + ID_PART_SEPARATOR + userId;
    }

    public static String convertToDelegateLayoutNodeId(final String portletEntityIdString) {
        return DELEGATE_LAYOUT_NODE_ID_PREFIX + portletEntityIdString.replace(ID_PART_SEPARATOR, DELEGATE_LAYOUT_NODE_ID_SEPARATOR);
    }
