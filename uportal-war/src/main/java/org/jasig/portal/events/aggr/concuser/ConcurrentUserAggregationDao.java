...
public interface ConcurrentUserAggregationDao<T extends ConcurrentUserAggregation> {
     ...
     */
    Set<T> getUnclosedConcurrentUserAggregations(DateTime start, DateTime end, AggregationInterval interval);
     ...
     */
    List<T> getConcurrentUserAggregations(DateTime start, DateTime end, AggregationInterval interval, AggregatedGroupMapping aggregatedGroupMapping, AggregatedGroupMapping... aggregatedGroupMappings);

    /**
     * @return All login aggregations for the date, time and interval
     */
    Set<T> getConcurrentUserAggregationsForInterval(DateDimension dateDimension, TimeDimension timeDimension, AggregationInterval interval);
    
    /**
     * Get a specific login aggregation
     */
    ConcurrentUserAggregation getConcurrentUserAggregation(DateDimension dateDimension, TimeDimension timeDimension, AggregationInterval interval, AggregatedGroupMapping aggregatedGroup);
}