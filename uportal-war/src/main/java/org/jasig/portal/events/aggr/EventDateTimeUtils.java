...
public final class EventDateTimeUtils {
     ...
     * @return Returns  0 if the instant is contained in the range: (getStart() <= instant < getEnd())
     *         Returns -1 if the range comes before the instant: (getEnd() <= instant)
     *         Returns  1 if the range comes after the instant (instant < getStart())
     * 
    ...
    /**
     * Find the {@link DateRange} that contains the specified instant. The first DateRange which contains
     * the instant is returned, null is returned if no range contains the instant.
     * 
    ...
    /**
     * Same function as {@link #findDateRange(ReadableInstant, Collection)} optimized for working on a pre-sorted List of
