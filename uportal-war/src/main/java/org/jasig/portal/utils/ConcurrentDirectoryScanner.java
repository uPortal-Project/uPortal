 ...
 * 
 * Note that the provided ExecutorService can never block on {@link ExecutorService#execute(Runnable)}
 * 
...
public final class ConcurrentDirectoryScanner implements DirectoryScanner {
    ...
    /**
     * @param processDirectories If directories should be passed to the fileProcessor {@link Function}, defaults to false.
     */
    ...
    protected <T> void scanDirectory(File directory, ConcurrentMap<File, T> results, FileFilter fileFilter, Function<Resource, T> fileProcessor) {
        final Queue<Tuple<File, Future<T>>> futures = new ConcurrentLinkedQueue<Tuple<File, Future<T>>>();
    ...
    protected <T> void recurseOnDirectory(
            ...
            final File directory, final FileFilter fileFilter, final Function<Resource, T> fileProcessor) {
    ...
    protected <T> void submitDirectoryRecurse(
        ...
        final Future<T> dirFuture = this.executorService.submit(new Callable<T>() {
            @Override
    ...
    protected <T> void submitProcessFile(final Queue<Tuple<File, Future<T>>> futures, final File child, final Function<Resource, T> fileProcessor) {
        final Future<T> fileFuture = this.executorService.submit(new Callable<T>() {
            @Override
            public T call() throws Exception {
    ...
    protected <T> void cleanFutures(final Queue<Tuple<File, Future<T>>> futures, final ConcurrentMap<File, T> results) {
