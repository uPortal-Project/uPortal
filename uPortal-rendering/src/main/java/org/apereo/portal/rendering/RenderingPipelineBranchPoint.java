...
/**
 * Offers an alternative to the "standard" rendering pipeline if certain conditions (encapsulated in a
 * <code>Predicate</code>) are satisfied.  If the specified <code>Predicate</code> returns <code>true</code>,
 * the alternate path will be followed.
 *
 * <p>This class is an extension point for uPortal adopters.  Beans of this type are automatically
 * detected and processed by the uPortal rendering infrastructure.  Multiple
 * <code>RenderingPipelineBranchPoint</code> beans will be ordered based on their <code>order</code> properties.
 * Beans with a lower order score are processed before beans with a higher order.
 *
 * @since 5.0
 */
public class RenderingPipelineBranchPoint implements Comparable<RenderingPipelineBranchPoint> {
     ...
     */
    public boolean renderStateIfApplicable(final HttpServletRequest request, final HttpServletResponse response)
        throws ServletException, IOException {

        final boolean rslt = predicate.test(request);
        if (rslt) {
            logger.debug("Using alternate pipe [{}] for branch point with order={}", alternatePipe, order);
            alternatePipe.renderState(request, response);
        } else {
            logger.debug("Bypassing alternate pipe [{}] for branch point with order={}", alternatePipe, order);
        }
        return rslt;
