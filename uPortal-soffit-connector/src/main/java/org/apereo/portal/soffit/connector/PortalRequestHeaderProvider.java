...
public class PortalRequestHeaderProvider extends AbstractHeaderProvider {
  ...
  public Header createHeader(RenderRequest renderRequest, RenderResponse renderResponse) {
    ...
    attributes.put(
        Attributes.SERVER_NAME.getName(), Collections.singletonList(renderRequest.getServerName()));
    attributes.put(
    ...
    final Header rslt =
        new BasicHeader(Headers.PORTAL_REQUEST.getName(), portalRequest.getEncryptedToken());
    logger.debug(
        "Produced the following PortalRequest header for username='{}':  {}", username, rslt);
