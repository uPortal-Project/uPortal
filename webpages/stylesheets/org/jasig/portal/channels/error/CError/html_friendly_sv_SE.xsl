  ...
  <xsl:template match="error">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td class="uportal-channel-subtitle" width="100%">Fel:<br/><span class="uportal-channel-error">
              <xsl:choose>
                <xsl:when test="@code='4'">Kanalen överskred maximal tidsgräns</xsl:when>
              <xsl:when test="@code='1'">Kanalen misslyckades rendera</xsl:when>
              <xsl:when test="@code='2'">Kanalen misslyckades initilaisera</xsl:when>
              <xsl:when test="@code='3'">Kanalen tog inte emot erforderlig information</xsl:when>
              <xsl:when test="@code='0'">Kanalen drabbades av ett allmänt fel</xsl:when>
              <xsl:when test="@code='5'">Kanalen tog inte emot PCS</xsl:when>
              <xsl:when test="@code='6'">Du har inte tillåtelse att se information i denna kanal</xsl:when>
              <xsl:when test="@code='7'">Kanalen är inte tillgänglig</xsl:when>
              <xsl:when test="@code='-1'">Kanalen drabbades av ett allmänt uPortal-fel</xsl:when></xsl:choose></span>
          <br/>
          <br/>
          ...
          <xsl:if test="$allowRefresh='true'">
            <a href="{$baseActionURL}?action=retry">
              ...
              <span class="uportal-label">Ladda om kanalen</span>
            </a>
          ...
          <xsl:if test="$allowReinstantiation='true'">
            <a href="{$baseActionURL}?action=restart">
              ...
              <img src="{$baseMediaURL}transparent.gif" border="0" width="4" height="4"/>
              <span class="uportal-label">Starta om kanalen</span>
            </a>
          </xsl:if>
        </td>
      </tr>
