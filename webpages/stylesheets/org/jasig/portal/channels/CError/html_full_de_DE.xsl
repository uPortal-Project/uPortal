...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="exception">
    <tr>
      ...
      <td width="100%" valign="top" align="left" class="uportal-channel-error">
        <xsl:choose>
          <xsl:when test="@code='-1'">Ausnahme zur Laufzeit(code -1)</xsl:when>
          <xsl:when test="@code='1'">Allgemeiner Übertragungsproblem (code 1)</xsl:when>
          <xsl:when test="@code='2'">Interne Abschaltung (code 2)</xsl:when>
          <xsl:when test="@code='3'">Berechtigungsproblem (code 3)</xsl:when>
          <xsl:when test="@code='4'">Fehlende Ressource (code 4)</xsl:when>
        </xsl:choose>
      </td>
    </tr>
    <xsl:if test="@code='2'">
