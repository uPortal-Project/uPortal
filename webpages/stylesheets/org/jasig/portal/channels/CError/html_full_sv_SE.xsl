...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="exception">
    <tr>
      ...
      <td width="100%" valign="top" align="left" class="uportal-channel-error">
        <xsl:choose>
          <xsl:when test="@code='-1'">Runtime-fel (kod -1)</xsl:when>
          <xsl:when test="@code='1'">Allmänt renderingsfel (kod 1)</xsl:when>
          <xsl:when test="@code='2'">Intern timeout (kod 2)</xsl:when>
          <xsl:when test="@code='3'">Auktoriseringsproblem (kod 4)</xsl:when>
          <xsl:when test="@code='4'">Resurs saknas (kod 4)</xsl:when>
        </xsl:choose>
      </td>
    </tr>
    <xsl:if test="@code='2'">
