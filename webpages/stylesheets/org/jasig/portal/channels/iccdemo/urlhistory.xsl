...
<xsl:for-each select="warning">
 ...
 <p>
 This is a history channel, part of the inter-channel communication demonstration. Channel shows last 10 URLs that have been viewed in the viewer channel. Click on a URL to view it again in the CViewer channel. Note that this will, in turn, cause CViewer to update the history.
 </p>
...
<p>
Hitting one of the URLs in the above list, will cause CHistory to pass URL to the CViewer.
One way of doing this is internally (through jndi contexts, directly to the CViewer). Another
is by using uPortal's URL syntax (uP_channelTarget) and passing a "url" parameter to the
CURLSelector, so that selector channel could then signal CViewer.
 <form action="{$baseActionURL}">
    <xsl:choose>
      <xsl:when test="$passExternally='true'">
        Currently passing URLs externally, using uP_channelTarget:
        <input type="submit" name="passExternally" class="uportal-button" value="Switch to pass internally"/>
      </xsl:when>
      <xsl:otherwise>
        Currently passing URLs internally to CViewer, using jndi:
