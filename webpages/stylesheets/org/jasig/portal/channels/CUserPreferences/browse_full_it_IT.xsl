...
<!-- end of the header table -->
  ...
  <form action="{$baseActionURL}" method="post">
    ...
    <table align="center" border="1" cellspacing="0" cellpadding="5">
      <tr>
        <th><input type="submit" name="moveTo" value="Sposta in..."/></th>
        <th>Riordina</th>
        <th>Nome</th>
        <th>Modifica</th>
      </tr>
  ...
  <xsl:if test="$modified = 'true'">
    ...
    <form action="{$baseActionURL}" method="post">
      ...
      <input type="submit" name="saveChanges" value="Salva i cambiamenti"/>    
    </form>
    </p><p/>
  </xsl:if>    
</xsl:template>
...
<xsl:template name="crumbTrail">Cartella: 
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>    
      <a href="{$baseActionURL}?action=browse&amp;folderID=root">I Miei Canali</a>
    </xsl:otherwise>
  </xsl:choose>
