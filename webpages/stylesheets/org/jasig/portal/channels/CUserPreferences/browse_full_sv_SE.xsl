...
<!-- end of the header table -->
  ...
  <form action="{$baseActionURL}" method="post">
    ...
    <table align="center" border="1" cellspacing="0" cellpadding="5">
      <tr>
        <th><input type="submit" name="moveTo" value="Flytta till..."/></th>
        <th>Sortera om</th>
        <th>Namn</th>
        <th>Ändra</th>
      </tr>
  ...
  <xsl:if test="$modified = 'true'">
    ...
    <form action="{$baseActionURL}" method="post">
      ...
      <input type="submit" name="saveChanges" value="Spara ändringar"/>    
    </form>
    </p><p/>
  </xsl:if>    
</xsl:template>
...
<xsl:template name="crumbTrail">Folder: 
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>    
      <a href="{$baseActionURL}?action=browse&amp;folderID=root">Mina kanaler</a>
    </xsl:otherwise>
  </xsl:choose>
