  	...
  	<td align="right">
          	<xsl:choose>
          		...
          		<xsl:when test="($group/@editable='true') and ($group/@canUpdate='true' or $group/@canAssignPermissions='true' or $group/@canManageMembers='true' or $group/@canCreateGroup='true')">
          			<a href="{$baseActionURL}?grpCommand=Lock&amp;grpCommandArg={$group/@id}"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/lock.gif" alt="グループを編集" title="グループを編集"/></a>
          		</xsl:when>
          	</xsl:choose>
			...
			<tr>	
				...
				<td nowrap="nowrap" colspan="2">
					<xsl:if test="$group/@canUpdate='true'">
						<input type="submit" onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Update';" value="更新" class="uportal-button" />
    ...
    <xsl:if test="($mode='edit') and ($grpServantMode='true')">
		...
		<input type="hidden" name="grpCommand" value="Cancel" />
		  <tr>
			...
			<td colspan="2">
			  <input type="submit" value="終了" class="uportal-button" />
			</td>
		  </tr>
		</form>
	</xsl:if>
