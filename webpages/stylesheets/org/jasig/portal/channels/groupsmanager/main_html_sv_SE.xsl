...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:xmsg="urn:x-lexica:xmsg:message:1.0">
  ...
  <xsl:template match="/">
		...
		function grpDeleteGroup(path){
			if (window.confirm('Är du säker att du vill ta bort denna grupp, alla gruppens rättigheter och alla medlemsskap??')){
				this.location.href=path;
			}
		}
		</SCRIPT>
        ...
        <xsl:if test="not($mode='edit')">
			...
			<form action="{$baseActionURL}">
				...
				<xsl:if test="$highlightedGroup[not(@id=0) and not(@searchResults='true')]"> 
					...
					<em>sök enbart i undergrupper till vald grupp</em>
					</td>
					</tr>
				</xsl:if>
			</form>
				  ...
				  <td colspan="5">
					...
					<xsl:if test="not($blockFinishActions)">
						<xsl:if test="count(descendant::*[@selected='true'])">
						  ...
						  <input type="submit" class="uportal-button"  onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Done';" value="Klar med urval" />
						</xsl:if>
					  <xsl:text>
					  </xsl:text>
					  <input type="submit" class="uportal-button"  onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Cancel';" value="Avbryt urval" />
					</xsl:if>
				  </td>
				</tr>
            </xsl:if>
  	...
  	<td align="right">
          	<xsl:choose>
          		...
          		<xsl:when test="$group/@searchResults='true'">
          			<a href="javascript:this.location.href='{$baseActionURL}?grpCommand=Delete&amp;grpCommandArg={$group/@id}';"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/delete.gif" alt="Ta bort grupp" title="Ta bort grupp"/></a>
          		</xsl:when>
          		<xsl:when test="$mode='edit'">
          			<a href="{$baseActionURL}?grpCommand=Unlock&amp;grpCommandArg={$group/@id}"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/unlock.gif" alt="Ändrat grupp färdigt" title="Ändrat grupp färdigt"/></a>
          			<xsl:if test="$group/@canDelete='true'">
						<a href="javascript:grpDeleteGroup('{$baseActionURL}?grpCommand=Delete&amp;grpCommandArg={$group/@id}');"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/delete.gif" alt="Ta bort grupp" title="Ta bort grupp"/></a>
					</xsl:if>
          		</xsl:when>
          		<xsl:when test="($group/@editable='true') and ($group/@canUpdate='true' or $group/@canAssignPermissions='true' or $group/@canManageMembers='true' or $group/@canCreateGroup='true')">
          			<a href="{$baseActionURL}?grpCommand=Lock&amp;grpCommandArg={$group/@id}"><img width="16" height="16" border="0" hspace="1" src="{$mediaBase}/lock.gif" alt="Ändra grupp" title="Ändra grupp"/></a>
          		</xsl:when>
          	</xsl:choose>
  ...
  <xsl:template name="rightPane">
			  ...
			  <xsl:if test="$group/@canCreateGroup='true'">
				  <tr>
					...
					<td colspan="2" nowrap="true">
					  <input type="submit" onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Create';" value="Skapa ny medlemsgrupp" class="uportal-button" />
					  <xsl:text>
					  </xsl:text>
    ...
    <xsl:if test="$mode='select'">
            <tr>
              <td colspan="2">
                <input type="submit" onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Select';"  value="Välj markerade" class="uportal-button" />
    ...
    <xsl:if test="($mode='edit') and ($grpServantMode='true')">
		...
		<input type="hidden" name="grpCommand" value="Cancel" />
		  <tr>
			...
			<td colspan="2">
			  <input type="submit" value="Klar" class="uportal-button" />
			</td>
		  </tr>
		</form>
	</xsl:if>
  ...
  <xsl:template name="propertiesDisplay">
    ...
    <tr><td></td>
        ...
        <xsl:choose>
                ...
                <xsl:otherwise>
                        <em>Det finns ingen ytterligare information tillgänglig</em>
                </xsl:otherwise>
        </xsl:choose>
        </td>
    </tr>
