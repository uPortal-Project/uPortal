			  ...
			  <xsl:if test="$group/@canCreateGroup='true'">
				  <tr>
					...
					<td colspan="2" nowrap="true">
					  <input type="submit" onClick="javascript:this.form.action='{$baseActionURL}?grpCommand=Create';" value="Izveidot jaunu grupu" class="uportal-button" />
					  <xsl:text>
					  </xsl:text>
      ...
      <xsl:if test="(($mode='edit') or ($mode='members')) and ($grpServantMode='true')">
		...
		<input type="hidden" name="grpCommand" value="Cancel" />
		  <tr>
			...
			<td colspan="2">
			  <input type="submit" value="Gatavs" class="uportal-button" />
			</td>
		  </tr>
		</form>
	</xsl:if>
