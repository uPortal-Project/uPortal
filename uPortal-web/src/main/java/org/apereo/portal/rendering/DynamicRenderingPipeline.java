...
public class DynamicRenderingPipeline implements IPortalRenderingPipeline {
    ...
    /**
     * The root element in the rendering pipeline. This element MUST only return {@link
     * CharacterEventTypes#CHARACTER} type events.
     */
    ...
    public void renderState(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        this.portalEventFactory.publishPortalRenderEvent(
                req, this, req.getPathInfo(), executionTime, portalRequestInfo);
    }
    ...
    protected String getMediaType(
        ...
        this.logger.warn(
                "No mediaType was specified in the pipeline output properties, defaulting to "
                        + DEFAULT_MEDIA_TYPE);
    ...
    public String toString() {
        return "DynamicRenderingPipeline using url syntax provider ["
                + this.urlSyntaxProvider
                + "] and wrapping pipeline component ["
                + this.pipeline
                + "].";
    }
}
