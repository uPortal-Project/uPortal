...
/**
 * A {@link javax.servlet.Filter} that enable client-side cross-origin requests by implementing
 * W3C's CORS (<b>C</b>ross-<b>O</b>rigin <b>R</b>esource <b>S</b>haring) specification for
 * resources. Each {@link HttpServletRequest} request is inspected as per specification, and
 ...
 * dependencies.
 *
 * <p>By default, it also sets following request attributes, that help to determine the nature of
 * the request downstream.
 *
 * <ul>
 *   <li><b>cors.isCorsRequest:</b> Flag to determine if the request is a CORS request. Set to
 *       <code>true</code> if a CORS request; <code>false</code> otherwise.
 *   <li><b>cors.request.origin:</b> The Origin URL, i.e. the URL of the page from where the request
 *       is originated.
 *   <li><b>cors.request.type:</b> Type of request. Possible values:
 *       <ul>
 *         <li>SIMPLE: A request which is not preceded by a pre-flight request.
 *         <li>ACTUAL: A request which is preceded by a pre-flight request.
 *         <li>PRE_FLIGHT: A pre-flight request.
 *         <li>NOT_CORS: A normal same-origin request.
 *         <li>INVALID_CORS: A cross-origin request which is invalid.
 *       </ul>
 ...
 * @see <a
 *     href="https://github.com/apache/tomcat/blob/trunk/java/org/apache/catalina/filters/CorsFilter.java">Apache
 *     Tomcat CorsFilter</a>
 */
public class CorsFilter implements Filter {
     ...
     * headers other than the simple response headers that the resource might use and can be
     * exposed.
     */
    ...
    /* from {@link GenericFilter} in Servlet 4 */
     ...
     * @see <a href="http://www.w3.org/TR/cors/#resource-requests">Simple Cross-Origin Request,
     ...
     *       <code>true</code> if CORS request; <code>false</code> otherwise.
     *   <li><b>cors.request.origin:</b> The Origin URL.
     *   <li><b>cors.request.type:</b> Type of request. Values: <code>simple</code> or <code>
     *       preflight</code> or <code>not_cors</code> or <code>invalid_cors</code>
     *   <li><b>cors.request.headers:</b> Request headers sent as 'Access-Control-Request-Headers'
     *       header, for pre-flight request.
     * </ul>
     *
     ...
     * @param decorateRequest A {@link String} representing a boolean whether to decorate the
     *     request object. The string "true" is parsed as true; any other value is false.
    ...
    private void parseAndStore(
        ...
        setAllowedHttpHeaders(allowedHttpHeaders);

        setExposedHeaders(exposedHeaders);

        setSupportsCredentials(supportsCredentials);
    ...
    public void setDecorateRequest(String decorateRequest) {
        log.debug("setDecorateRequest set to {}", decorateRequest);
        // For any value other then 'true' this will be false.
        this.decorateRequest = Boolean.parseBoolean(decorateRequest);
    }

    public void setPreflightMaxAge(String preflightMaxAge) throws ServletException {
        log.debug("setPreflightMaxAge set to {}", preflightMaxAge);
        try {
    ...
    public void setSupportsCredentials(String supportsCredentials) {
        log.debug("setSupportsCredentials set to {}", supportsCredentials);
        // For any value other then 'true' this will be false.
        this.supportsCredentials = Boolean.parseBoolean(supportsCredentials);
    }

    public void setExposedHeaders(String exposedHeaders) {
        log.debug("setExposedHeaders set to {}", exposedHeaders);
        Set<String> setExposedHeaders = parseStringToSet(exposedHeaders);
        this.exposedHeaders.clear();
        this.exposedHeaders.addAll(setExposedHeaders);
    }

    public void setAllowedHttpHeaders(String allowedHttpHeaders) {
        log.debug("setAllowedHttpHeaders set to {}", allowedHttpHeaders);
    ...
    public void setAllowedOrigins(String allowedOrigins) {
        log.debug("setAllowedOrigins set to {}", allowedOrigins);
    ...
    /**
     * The Access-Control-Max-Age header indicates how long the results of a preflight request can
    ...
    /**
     * The Vary header indicates allows disabling proxy caching by indicating the the response
    ...
    protected enum CORSRequestType {
        ...
        ACTUAL,
        /**
         * A pre-flight CORS request, to get meta information, before a non-simple HTTP request is
         * sent.
         */
        PRE_FLIGHT,
        ...
        NOT_CORS,
        /**
         * An invalid CORS request, i.e. it qualifies to be a CORS request, but fails to be a valid
         * one.
         */
        INVALID_CORS
    }
