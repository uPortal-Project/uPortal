...
public class MediaManager {
	...
	try {
	    ...
	    if(url!=null)  mediaProps=new OrderedProps(url.openStream());
    ...
    {  
	String CURRENTDIR= System.getProperty("user.dir")+System.getProperty("file.separator");
	if (uri == null) uri="file://"+ CURRENTDIR + "mime.properties";
	try {
	    URL url=expandSystemId(uri);
	    if(url!=null)  mimeProps=new OrderedProps(url.openStream());
	} catch (IOException ioe1) {
    ...
    {  
	String CURRENTDIR= System.getProperty("user.dir")+System.getProperty("file.separator");
	if (uri == null) uri="file://"+ CURRENTDIR + "serializer.properties";
	try {
	    URL url=expandSystemId(uri);
	    if(url!=null)  serializerProps=new OrderedProps(url.openStream());
	} catch (IOException ioe1) {
	...
	if(mediaProps!=null) return mediaProps.getValue(req.getHeader("user-Agent"));
	return (String) null;
    }
	...
	if(mimeProps==null) this.setMimeProps((String) null);
	
	if(mimeProps!=null) return mimeProps.getValue(mediaType);
	else return null;
    }
	...
	if(serializerProps==null) this.setSerializerProps((String) null);
	
	if(serializerProps!=null) serializerName=serializerProps.getValue(mediaType);
