...
public class CHistory extends BaseChannel {
    ...
    public void setStaticData(ChannelStaticData sd) throws PortalException {
        ...
        try {
            ...
            LogService.instance().log(LogService.ERROR, "CHistory.getUserXML(): Could not find subcontext /channel-obj in JNDI");
        ...
        try {
            ...
            LogService.instance().log(LogService.ERROR, "CHistory.getUserXML(): Could not bind channel object for channel id="+staticData.getChannelSubscribeId());
        } catch (NamingException e) {
            LogService.instance().log(LogService.ERROR, e);
        }
    }
    ...
    private Document getUserXML() {
        ...
        if(viewerId==null) {
            ...
            if(bo==null) {
                Element warningEl=doc.createElement("warning");
                warningEl.appendChild(doc.createTextNode("Viewer channel found, but no object was found bound in viewer's jndi context. Perhaps viewer should be moved ot the same tab."));
                urlselectorEl.appendChild(warningEl);
            }
        }
