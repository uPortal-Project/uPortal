    ...
    extends RDBMUserLayoutStore
{
    ...
    private boolean layoutExistsForUser(IPerson person) {
        ...
        final int struct_count = jdbcTemplate.queryForInt("SELECT COUNT(*) FROM up_layout_struct WHERE user_id = ?", new Object[] { Integer.valueOf(person.getID()) });
    ...
    public org.dom4j.Element exportLayout(IPerson person, UserProfile profile) {
        ...
        try {
            ...
            String msg = "Unable to obtain layout & profile for user '" 
                            + person.getAttribute(IPerson.USERNAME) + "', profileId " 
        ...
        if (log.isDebugEnabled()) {
            ...
            try {
                ...
                throw new RuntimeException("Failed to write the layout for user '" 
                            + person.getAttribute(IPerson.USERNAME) + "' to the DEBUG log", t);
            }
            log.debug("Layout for user:  " + person.getAttribute(IPerson.USERNAME) 
        ...
        List structFolderAttrNames = Collections.list(ssup.getFolderAttributeNames());
        for (Iterator fldrs = (Iterator) layoutDoc.selectNodes("//folder").iterator(); fldrs.hasNext();) {
            org.dom4j.Element fld = (org.dom4j.Element)fldrs.next();
            for (Iterator attrItr = structFolderAttrNames.iterator(); attrItr.hasNext(); ) {
        ...
        List structChannelAttrNames = Collections.list(ssup.getChannelAttributeNames());
        for (Iterator chnls = (Iterator) layoutDoc.selectNodes("//channel").iterator(); chnls.hasNext();) {
            org.dom4j.Element chnl = (org.dom4j.Element)chnls.next();
            for (Iterator attrItr = structChannelAttrNames.iterator(); attrItr.hasNext(); ) {
                String attr = (String)attrItr.next();
        ...
        List themeChannelAttrNames = Collections.list(tsup.getChannelAttributeNames());
        for (Iterator chnls = (Iterator) layoutDoc.selectNodes("//channel").iterator(); chnls.hasNext();) {
            org.dom4j.Element chnl = (org.dom4j.Element)chnls.next();
            for (Iterator attrItr = themeChannelAttrNames.iterator(); attrItr.hasNext(); ) {
                String attr = (String)attrItr.next();
        ...
        Iterator locale = (Iterator) layoutDoc.selectNodes("//@locale").iterator();
        while (locale.hasNext()) {
            org.dom4j.Attribute loc = (org.dom4j.Attribute)locale.next();
            loc.getParent().remove(loc);
        }

        // (3) Scrub unnecessary channel information...
        for (Iterator orphanedChannels = (Iterator) layoutDoc.selectNodes("//channel[@fname = '']").iterator(); orphanedChannels.hasNext();) {
            ...
            org.dom4j.Element ch = (org.dom4j.Element)orphanedChannels.next();
            ch.getParent().remove(ch);
        }
        List channelAttributeWhitelist = Arrays.asList(new String[] { 
                        "fname", 
                        "unremovable", 
                        "hidden", 
                        "immutable",
        ...
        Iterator channels = (Iterator) layoutDoc.selectNodes("//channel").iterator();
        while (channels.hasNext()) {
            org.dom4j.Element oldCh = (org.dom4j.Element)channels.next();
            org.dom4j.Element parent = oldCh.getParent();
            org.dom4j.Element newCh = fac.createElement("channel");
            for (Iterator attrItr = channelAttributeWhitelist.iterator(); attrItr.hasNext(); ) {
                String aName = (String)attrItr.next();
        ...
        for (Iterator origins = (Iterator) layoutDoc.selectNodes("//@dlm:origin").iterator(); origins.hasNext();) {
            org.dom4j.Attribute org = (org.dom4j.Attribute)origins.next();
        ...
        for (Iterator it = (Iterator) layoutDoc.selectNodes("//@dlm:target").iterator(); it.hasNext();) {
            org.dom4j.Attribute target = (org.dom4j.Attribute)it.next();
        ...
        for (Iterator names = (Iterator) layoutDoc.selectNodes("//dlm:*/@name").iterator(); names.hasNext();) {
            org.dom4j.Attribute n = (org.dom4j.Attribute)names.next();
        ...
        if (!isFragmentOwner(person)) {
            ...
            // (5) Remove dlm:plfID...
            for (Iterator plfid = (Iterator) layoutDoc.selectNodes("//@dlm:plfID").iterator(); plfid.hasNext();) {
                org.dom4j.Attribute plf = (org.dom4j.Attribute)plfid.next();
                plf.getParent().remove(plf);
            }

            // (6) Remove database Ids...
            for (Iterator ids = (Iterator) layoutDoc.selectNodes("//@ID").iterator(); ids.hasNext();) {
                org.dom4j.Attribute a = (org.dom4j.Attribute)ids.next();
