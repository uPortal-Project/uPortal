...
public class LayoutRestrictionFactory {
 ...
 static {
  try {	
 	 activeRestrictions = ResourceLoader.getResourceAsProperties(LayoutRestrictionFactory.class,RESTRICTIONS_PATH);
  } catch ( Exception e ) {
  	 throw new RuntimeException ( "init: " + e.toString() ); 
  }
 }

 private static ILayoutRestriction getRestriction ( String className ) throws PortalException {
 	  ...
 	  try {	
 	 	ILayoutRestriction restriction = (ILayoutRestriction) Class.forName(className).newInstance();
 ...
 private static ILayoutRestriction createRestriction( String restrictionType ) throws PortalException {
 ...
 public static ILayoutRestriction createRestriction( RestrictionType restrictionType ) throws PortalException {
 	return createRestriction(restrictionType.getType());
 }
 
 public static ILayoutRestriction createRestriction( RestrictionType restrictionType, String restrictionValue, RestrictionPath restrictionPath ) throws PortalException {
 	ILayoutRestriction restriction = createRestriction ( restrictionType );
 	restriction.setRestrictionExpression(restrictionValue);
 	restriction.setRestrictionPath(restrictionPath);
 	return restriction;
 }
 
 public static ILayoutRestriction createRestriction( RestrictionType restrictionType, String restrictionValue ) throws PortalException {
