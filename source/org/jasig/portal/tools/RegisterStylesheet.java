...
public class RegisterStylesheet {
  ...
  private static void setPortalBaseDir() {
    ...
    if (portalBaseDirParam != null)
    {
      ...
      GenericPortalBean.setDbImpl(new DBImpl());
    }
    else
    {
  ...
  public static void main(String[] args) {
    ...
    ICoreStylesheetDescriptionDB csdb=new CoreStylesheetDescriptionDBImpl();
	...
	if(success) {
	    ...
	    System.out.println("Update failed!"); return; 
	}
    } else {
	Integer id=null;
	if(isTheme) id=csdb.addThemeStylesheetDescription(stylesheetDescriptionURI,stylesheetURI);
	else id=csdb.addStructureStylesheetDescription(stylesheetDescriptionURI,stylesheetURI);
	if(id==null) { System.out.println("Save failed!"); return; } else {
	...
	// verify
	if(isTheme) {
	    ThemeStylesheetDescription tsd=GenericPortalBean.getDbImplObject().getThemeStylesheetDescription(stylesheetId);
	} else {
	    StructureStylesheetDescription ssd=GenericPortalBean.getDbImplObject().getStructureStylesheetDescription(stylesheetId);
	}
