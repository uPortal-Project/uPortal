...
public class CBookmarks extends GenericPortalBean implements IXMLChannel
{
  ...
  //String uri="file://"+getPortalBaseDir () + "source" + fs + "org" + fs + "jasig" + fs + "portal" + fs + "xmlchannels"+fs + "bookmarks.xml";
  //String uri = "http://localhost:8080/xmlchannels/bookmarks.xml";
  ...
  protected Document getBookmarkXML (ChannelRuntimeData rd)
  {
    ...
    {  //If there is no bookmarks, then go and get it from the database
      ...
      try
      {
        ...
        ResultSet statem =
        con.createStatement ()
        .executeQuery ("SELECT BOOKMARK_XML FROM PORTAL_BOOKMARKS WHERE PORTAL_USER_ID="+ userid);
        ...
        else
        {
          ...
          .executeQuery ("select portal_bookmarks.bookmark_xml from portal_bookmarks , portal_users where portal_users.user_name = 'default' and portal_users.id = portal_bookmarks.portal_user_id ");
          statem.next ();
          inputXML = statem.getString ("BOOKMARK_XML");
  ...
  protected void saveXML (ChannelRuntimeData rd)
  {
    if ( bookmarksXML != null)
    {
      ...
      try
      {
        ...
        statem.executeUpdate ("update portal_bookmarks set bookmark_xml = '"+ outString.toString ()
        +"' where portal_user_id = " + GetUserID (rd.getHttpRequest ()) );
      }
      catch (Exception e)
      {
        Logger.log (Logger.ERROR,e);
  ...
  public void setRuntimeData (ChannelRuntimeData rd) {
    ...
    if ( (action=runtimeData.getParameter ("action"))!=null) {
      ...
      else if (action.equals ("saveBookmark"))
      {
        ...
        //runtimeData.setParameter ("url",makeUrlSafe(runtimeData.getParameter ("url")));
