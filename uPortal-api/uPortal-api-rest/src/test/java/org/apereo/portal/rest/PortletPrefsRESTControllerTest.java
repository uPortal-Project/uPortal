...
public class PortletPrefsRESTControllerTest {
    ...
    @InjectMocks
    private PortletPrefsRESTController portletPrefsRESTController;
    ...
    public void setUp() throws Exception {
        ...
        Mockito.when(portletDao.getPortletDefinitionByFname(not_authorized_name)).thenReturn(not_authorized_portletDefinitionMock);
        Mockito.when(not_authorized_portletDefinitionMock.getPortletDefinitionId()).thenReturn(not_authorized_portletDefIDMock);
        Mockito.when(portletEntityRegistry.getOrCreateDefaultPortletEntity(req, not_authorized_portletDefIDMock)).thenThrow(
            new IllegalArgumentException( "No portlet definition found for id '200'."));

        //authorized to subscribe common methods
        Mockito.when(portletDao.getPortletDefinitionByFname(valid_name)).thenReturn(valid_portletDefinitionMock);
        Mockito.when(valid_portletDefinitionMock.getPortletDefinitionId()).thenReturn(valid_portletDefIDMock);
        Mockito.when(portletEntityRegistry.getOrCreateDefaultPortletEntity(req, valid_portletDefIDMock)).thenReturn(valid_entityMock);
    ...
    public void getEntityNotAuthorized() {
        ...
        ResponseEntity response = portletPrefsRESTController.getEntity(req,res,not_authorized_name);
        Assert.assertEquals(403,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: User not authorized to access portlet '"+not_authorized_name+"'.",(String)response.getBody());
    }

    @Test
    public void getCompositePrefsNonExistent() {

        ResponseEntity response = portletPrefsRESTController.getCompositePrefs(req,res,non_existant_name);
        Assert.assertEquals(404,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: Portlet not found",(String)response.getBody());
    }
    ...
    public void getCompositePrefsNotAuthorized() {
        ...
        ResponseEntity response = portletPrefsRESTController.getCompositePrefs(req,res,not_authorized_name);
        Assert.assertEquals(403,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: User not authorized to access portlet '"+not_authorized_name+"'.",(String)response.getBody());
    }
    ...
    public void getCompositePrefs() {
        ...
        ResponseEntity response = portletPrefsRESTController.getCompositePrefs(req,res,valid_name);
        Assert.assertEquals(200,response.getStatusCodeValue());
        JsonNode body=null;
        try {
        ...
        Assert.assertTrue("Json is not equal, \nExpected: "+jsonmap.toString()+"\nActual: "+body.toString(),jsonmap.equals(body));
    ...
    public void getEntityOnlyPrefsNonExistant() {
        ResponseEntity response = portletPrefsRESTController.getEntityOnlyPrefs(req,res,non_existant_name);
        Assert.assertEquals(404,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: Portlet not found",(String)response.getBody());
    }
    ...
    public void getEntityOnlyPrefs() {
        ...
        Mockito.when(portletPreferencesFactory.createAPIPortletPreferences(req,valid_entityMock,false,true))
            .thenReturn(defPrefs);
        ...
        ResponseEntity response = portletPrefsRESTController.getEntityOnlyPrefs(req,res,valid_name);
        Assert.assertEquals(200,response.getStatusCodeValue());
        JsonNode body=null;
        try {
            body=mapper.readTree((String)response.getBody());
    ...
    public void getDefinitionPrefsNoSession() {
        ...
        ResponseEntity response = portletPrefsRESTController.getDefinitionPrefs(req2,res,valid_name);
        Assert.assertEquals(404,response.getStatusCodeValue());
    }
    ...
    public void getDefinitionPrefsSessionNotAuthenticated() {
        ...
        ResponseEntity response = portletPrefsRESTController.getDefinitionPrefs(req2,res,valid_name);
        Assert.assertEquals(401,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: must be logged in to use this action.",(String)response.getBody());
    }

    @Test
    public void getDefinitionPrefsSessionNonExistant() {

        ResponseEntity response = portletPrefsRESTController.getDefinitionPrefs(req,res,non_existant_name);
        Assert.assertEquals(404,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: Portlet not found",(String)response.getBody());
    }
    ...
    public void getDefinitionPrefsSessionNonConfig() {
        //can't programatically test this one without setting up a full stack since it calls the static method:
        //      AuthorizationPrincipalHelper.principalFromUser(user)
        //to insure that only portal administrators or others with configuration authority can configure.
    ...
    public void getDefinitionPrefs() {
        //can't progamatically test this one without setting up a full stack since it calls the static method:
        //      AuthorizationPrincipalHelper.principalFromUser(user)
        //to insure that only portal administrators or others with configuration authority can configure.
        // *Could easily be implemented by using PowerMock
        //has been thouroughly tested through manual testing though
    ...
    public void putDefinitionPrefsNoSession() {
        ...
        ResponseEntity response = portletPrefsRESTController.putDefinitionPrefs(req2,res,valid_name,body);
        Assert.assertEquals(404,response.getStatusCodeValue());
    }
    ...
    public void putDefinitionPrefsSessionNonExistant() {
        String body="{\"pref1\":\"value1\"}";
        ResponseEntity response = portletPrefsRESTController.putDefinitionPrefs(req,res,non_existant_name,body);
        Assert.assertEquals(404,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: Portlet not found",(String)response.getBody());
    }
    ...
    public void putEntityPrefsNonExistant() {
        String body="{\"pref1\":\"bob\"}";
        ResponseEntity response = portletPrefsRESTController.putEntityPrefs(req,res,non_existant_name,body);
        Assert.assertEquals(404,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: Portlet not found",(String)response.getBody());
    }
    ...
    public void putEntityPrefsNotAuthorized() {
        String body="{\"pref1\":\"bob\"}";
        ResponseEntity response = portletPrefsRESTController.putEntityPrefs(req,res,not_authorized_name,body);
        Assert.assertEquals(403,response.getStatusCodeValue());
        Assert.assertEquals("ERROR: User not authorized to access portlet '"+not_authorized_name+"'.",(String)response.getBody());
    }
    ...
    public void putEntityPrefsReadOnly() throws Exception {
        ...
        Mockito.doThrow(new ReadOnlyException("Preference '" + "pref1" + "' is read only")).when(prefs).setValues("pref1", values);
