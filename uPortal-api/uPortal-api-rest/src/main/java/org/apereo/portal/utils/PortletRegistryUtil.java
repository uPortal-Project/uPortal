 ...
 *
 * - matches(query, portlet) tests the query against protlet title, name and description
 *
 * - buildPortletUrl(servletRequest, portletDef) builds a portal URL to the specific portlet
 *       if the remote user in the request has browse permissions
 */
@Component
public class PortletRegistryUtil {
     ...
     * @return          true if the query string is contained in the above 3 attributes;
     *                  otherwise, false
     */
    ...
    /**
     *  Builds a portal URL to the specific portlet definition {@link IPortletDefinition},
     *  if the remote user in the request has browse permissions to the portlet.
     *
    ...
    public String buildPortletUrl(HttpServletRequest httpServletRequest, IPortletDefinition portlet) {
        final IPortletWindow portletWindow =
            this.portletWindowRegistry
                .getOrCreateDefaultPortletWindowByFname(httpServletRequest, portlet.getFName());
